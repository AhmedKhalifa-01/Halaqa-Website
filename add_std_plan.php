...
if((isset($_SESSION['email'])!= "") && (isset($_SESSION['staff_job']) == "JOB_01")) {
	...
	if(isset($_POST['submit'])){
		...
		$sql = "INSERT INTO `std_plan`(`std_id`, `c_id`, `versus`, `start_date`, `end_date`) VALUES ('".$_GET['std_id']."','".$_GET['c_id']."','".$verse."','".$startDate."','".$endDate."')";
		$result = mysqli_query($conn,$sql);
		if(!$result){
			echo "<script>alert('خطأ في إدخال الدرجة')</script>";
		}else{
			echo "<script>alert('OK')</script>";
			echo "<script>window.location.href='course_std_details.php?id=".$_GET['c_id']."';</script>";
		}
	}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إدارة الخطة</title>
						...
						$row = mysqli_fetch_assoc($result);
							...
							<article class="card">
									...
									<div class="card-footer">
										<form class="login-form" action = "add_std_plan.php?std_id='.$_GET['std_id'].'&c_id='.$_GET['c_id'].'" method = "post"/>
											<input type = "text" placeholder="إدخال الأجزاء المراد حفظها في الفترة أدناه" name="verse"/>
											<p style="color:red;font-size: 12px;width: 100%;">قم باستعمال الفاصلة للفصل بين الأجزاء مثال : 1,2,14,25</p>
											<input type = "date" placeholder="تاريخ البدء" name="start_date"/>
											<input type = "date" placeholder="تاريخ الانتهاء" name="end_date"/>
