					...
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>إضافة طلبة لحلقة المتون</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="add_to_existing_mton_ring.php?mton_ring_id=<?php echo $_GET['mton_ring_id']; ?>" method="post">
								<p>الحلقة</p>
								<select name="staff" id="staff-select">
									<?php
										$sql = "SELECT * FROM mton_ring WHERE mton_ring_id = '".$_GET['mton_ring_id']."'";
								...
								<p>اختر الطلبة لإضافتهم لحلقة المتون : </p>
								<ul>
									<?php
									// Retrieve the list of students from the database
									$sql = "SELECT *
											FROM students WHERE state != 'مفصول'
											AND mton_ring_id != ".$_GET['mton_ring_id']."
											AND mton_ring_id_02 != ".$_GET['mton_ring_id']."
											AND mton_ring_id_03 != ".$_GET['mton_ring_id']."";
									$result = mysqli_query($conn, $sql);
