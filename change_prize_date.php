...
/* adding new course */
	...
	if(isset($_POST['add_course'])){
		...
		$sql = "UPDATE `prize` SET `start_date`='".$start_date."',`end_date`='".$end_date."' WHERE prize_id = ".$_GET['id']."";
		$result = mysqli_query($conn,$sql);
		if($result){
			echo "<script>window.location.href='prize_man.php';</script>";
		}else{
			echo "<script>alert('خطأ في تعديل المدة ')</script>";
			echo "<script>window.location.href='prize_man.php';</script>";
		}
	}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>تغيير زمن الجائزة</title>
					...
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>تغيير زمن الجائزة</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="change_prize_date.php?id=<?php echo $_GET['id']?>" method="post" enctype="multipart/form-data">
								<input type="date" placeholder="تاريخ بدء الجائزة" name="start_date"/>
								<input type="date" placeholder="تاريخ انتهاء الجائزة " name="end_date"/>
