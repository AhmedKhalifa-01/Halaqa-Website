...
/* checking if an admin is logged in */
	...
	if(isset($_GET['ring_id'])){
		...
		while($row = mysqli_fetch_assoc($result)){
			$sql = "SELECT std_id FROM course_details WHERE std_id = '".$row['std_id']."' AND course_id = '".$_GET['c_id']."'";
			if(mysqli_num_rows(mysqli_query($conn,$sql)) == 0){
				$sql = "INSERT INTO `course_details`(`course_id`, `std_id`) VALUES ('".$_GET['c_id']."','".$row['std_id']."')";
							...
							while($row = mysqli_fetch_assoc($result)){
								if($row['student_count'] > 0){
									echo '<article class="card">
											...
											<div class="card-body">
												<p> اسم الاستاذ : '.$row['teacher_name'].'</p>';
												if($_SESSION['staff_job'] == 'JOB_01'){
											...
											<div class="card-footer">
												<a href="add_course_ring.php?ring_id='.$row['ring_id'].'&c_id='.$_GET['c_id'].'" onclick="return confirmAdd()">إضافة الحلقة للدورة</a>
											</div>';
...
<!-- partial -->
  ...
  <script>
		function confirmAdd() {
			if (confirm("هل أنت متأكد من إضافة الحلقة للدورة ؟")) {
				return true;
			} else {
				return false;
			}
