...
/* checking if an admin is logged in */
	...
	if(isset($_GET['std_id'])){
		...
		$sql = "INSERT INTO `prize_participating_students`(`prize_id`,`std_id`) VALUES ('".$_GET['prize_id']."','".$_GET['std_id']."')";
		$result = mysqli_query($conn,$sql);
		if(!$result){
			echo "<script>alert('خطأ في إضافة الطالب للجائزة')</script>";
			echo "<script>window.location.href='prize_man.php';</script>";
		}else{
			$sql = "UPDATE prize SET no_std = no_std+1 WHERE prize_id = '".$_GET['prize_id']."'";
			$result = mysqli_query($conn,$sql);
			if($result){
				echo "<script>alert('تمت إضافة الطالب للجائزة')</script>";
				echo "<script>window.location.href='add_prize_std.php?prize_id=".$_GET['prize_id']."';</script>";
			}
		}
	}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إضافة طالب للجائزة</title>
							...
							if($result){
								while($row = mysqli_fetch_assoc($result)){
									echo '<tr>';
										...
										$sql = "SELECT * FROM prize_participating_students WHERE std_id = '".$row['std_id']."' AND prize_id = '".$_GET['prize_id']."'";
										if(mysqli_num_rows(mysqli_query($conn,$sql)) > 0 and $_SESSION['staff_job'] == 'JOB_01' ){
											...
											echo '<td>
													<form class="login-form" action="add_prize_std.php?std_id='.$row['std_id'].'&prize_id='.$_GET['prize_id'].'" method="post">
														<input style="font-size:18px;font-weight:700;" type="submit" value="إضافة" name="add"/>
													</form>
												</td>';
										}
									echo '</tr>';
