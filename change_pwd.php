...
/* adding new course */
	if(isset($_POST['submit'])){
		...
		if($oldpass == mysqli_fetch_assoc($result)['pass']){
			if($pass == $rpass){
				$sql = "UPDATE `staff` SET `pass`='".$pass."' WHERE staff_id = '".$_SESSION['email']."'";
				$result = mysqli_query($conn,$sql);
				if($result){
					echo "<script>alert('تم تغيير كلمة المرور بنجاح')</script>";
					echo "<script>window.location.href='change_pwd.php';</script>";
				}else{
...
<header class="header">
	...
	<div class="header-logo">
			...
			<?php if($_SESSION['staff_job'] != "JOB_03"){?>
					...
					?>
					<a href="change_pwd.php" class="active">تغيير كلمة المرور</a>
					<?php
						if($_SESSION['staff_job'] == 'JOB_01'){
				...
				<div class="card-grid">
					<article class="card">
						<div class="card-header">
							<div>
								<span><img src="imgs/icons/padlock.png" /></span>
								<h3>تغيير كلمة المرور</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="change_pwd.php" method="post">
