...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إدارة المعلمين</title>
	...
	<div class="responsive-wrapper">
		...
		<div class="content-header">
			<div class="content-header-intro">
				...
				<?php if($_SESSION['staff_job'] == 'JOB_01'){ ?>
				<center><h3 style="font-weight:bold;font-size:24px;">عدد المعلمين في المجمع : <?php 
														$res = mysqli_query($conn,"SELECT COUNT(staff_id) AS count FROM staff WHERE staff_job = 'JOB_03'"); 
			...
			<div class="content-header-actions">
				...
				<?php
					if($_SESSION['staff_job'] == 'JOB_01'){
						echo '<a href="staff_att_record.php" class="button">
		...
		<div class="content">
			...
			<div class="content-main">
				<div class="search">
					<form action="staff_management.php?search=1" methode="get">
						<input type="text" placeholder="اكتب اسم المعلم" name = "search"/>
						<button type="submit">
				...
				<div class="card-grid">
					<?php
						...
						$sql = "SELECT * FROM staff WHERE staff_id = '".$_SESSION['email']."'";
						
						if($_SESSION['staff_job'] == 'JOB_01'){
							$sql = "SELECT * FROM staff WHERE staff_job = 'JOB_03'";
						}
						if($_SESSION['staff_job'] == 'JOB_02'){
							$sql =" SELECT * FROM staff
									INNER JOIN teacher_ring ON teacher_ring.teacher_id = staff.staff_id
						...
						if(isset($_GET['search'])){
							$sql = "SELECT * FROM staff WHERE staff_job = 'JOB_03' AND staff_name LIKE '%".$_GET['search']."%'";
							if($_SESSION['staff_job'] == 'JOB_02'){
								$sql =" SELECT * FROM staff
										INNER JOIN teacher_ring ON teacher_ring.teacher_id = staff.staff_id
										INNER JOIN ring ON ring.ring_id = teacher_ring.ring_id
										INNER JOIN ring_admins ON ring.ring_id = ring_admins.ring_id
										WHERE ring_admins.staff_id = '".$_SESSION['email']."' AND staff_name LIKE '%".$_GET['search']."%'
									";
							}
						}
								...
								echo '<article class="card">
										<div class="card-header">
											...
											if($row['staff_job'] == 'JOB_03'){
												echo '<span><img src="imgs/icons/parent.png"/></span>';
											}
											if($row['isTemp'] == 'محتسب'){
												echo '<h3 id="test">'.$row['staff_name'].' - معلم محتسب</h3>';
											}else{
										...
										<div class="card-body">';
											...
											<p>الحلقات : </p>
											';
											$sql = "SELECT ring.ring_name,ring.ring_id 
													FROM ring
													INNER JOIN teacher_ring tr ON tr.ring_id = ring.ring_id
													WHERE tr.teacher_id = '".$row['teacher_id']."'";
											...
											if(mysqli_num_rows($re) > 0){
												...
												while($rrow = mysqli_fetch_assoc($re)){
													...
													echo '<li style="padding:5px;font-size: 15px;">'.$rrow['ring_name'].'</li>';
												}
												echo '</ol>';
											}
											if($row['status'] != 'إجازة'){
										...
										<div class="card-footer">
											<a href="staff_more_details.php?id='.$row['staff_id'].'&type=teacher">مزيد من التفاصيل</a>										</div>
										<div class="card-footer">
											<a href="staff_change_details.php?staff_id='.$row['staff_id'].'&type=teacher">تعديل البيانات</a>
										</div>';
										if($row['status'] == 'إجازة'){
											echo '<div class="card-footer">
													<a href="staff_management.php?staff_id='.$row['staff_id'].'&back=1">عودة من الإجازة</a>
												</div>';
										}else{
											echo '<div class="card-footer">
													<a href="staff_management.php?staff_id='.$row['staff_id'].'&vec=1">إجازة للمعلم</a>
												</div>';
										}
										echo '
									</article>
