	...
	if(isset($_POST['move_std'])){
		...
		if (!empty($students)) {
			foreach ($students as $std_id) {
				$sql = "SELECT ring_id FROM students WHERE std_id = '".$std_id."'";
				$result = mysqli_query($conn,$sql);
				$row = mysqli_fetch_assoc($result);
				if($row['ring_id'] == $_GET['ring_id']){
					mysqli_query($conn,"UPDATE students SET temp_ring_id = ring_id WHERE std_id = '".$std_id."'");
				}else{
					mysqli_query($conn,"UPDATE students SET temp_ring_id = '".$ring."' WHERE std_id = '".$std_id."'");
				}
				echo "<script>alert('تم تحويل الطالب بنجاح');</script>";
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="ring_move_std.php" method="post">
								<p>اختر الحلقة لتحويل الطلبة إليها</p>
								<select name="ring" id="staff-select">
									<?php
										/*$sql = "SELECT *,staff.staff_name FROM ring
												INNER JOIN staff ON ring.staff_id = staff.staff_id
										 		WHERE ring_id != '".$_GET['ring_id']."'";*/
								...
								<ul>
									...
									$sql = "SELECT students.std_id,students.ring_id,students.temp_ring_id,ring.ring_id,students.std_name FROM ring 
											INNER JOIN students ON ring.ring_id = students.ring_id
											WHERE students.temp_ring_id = '".$_GET['ring_id']."' ORDER BY std_name
									";
									$result = mysqli_query($conn, $sql);
									...
									if(mysqli_num_rows($result) > 0) {
										while($row = mysqli_fetch_assoc($result)) {
											if($row['ring_id'] != $row['temp_ring_id']){
