...
/* adding new post */
	...
	if(isset($_POST['add_news'])){
		...
		$fileExtensionsAllowed = ['pdf']; // Only allow PDF files
		...
		if (!in_array($fileExtension, $fileExtensionsAllowed)) { // Check if file extension is allowed
		...
		if (empty($errors)) { // If no errors, move uploaded file to new location
			$didUpload = move_uploaded_file($fileTmpName, $uploadPath);
				
			if ($didUpload) {
				mysqli_query($conn,"DELETE FROM tajweed WHERE 1");
				$pdf = basename($fileName);
				$sql = "INSERT INTO tajweed VALUES ('".$fileName."')";
				$result = mysqli_query($conn,$sql);
				
				if($result){
					echo "<script>window.location.href='tajweed_man.php';</script>";
				}else{
		...
		if (!empty($errors)) { // If errors occurred, display error messages
			foreach ($errors as $error) {
				//echo "<p>Error: $error</p>";
			}
			echo "<script>alert('الرجاء رفع ملف pdf والمحاولة مرة اخرى')</script>";
		}
		//}
	}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>ملف التجويد</title>
	...
	<div class="responsive-wrapper">
		...
		<div class="content">
			...
			<div class="content-main">
				<center style="margin-bottom:20px;"><a href="homepage_man.php" class="button" style="width:100px;height:50px;">
					<span>رجوع</span>
				</a></center>
				<div class="card-grid">
					<article class="card">
						<div class="card-header">
							<div>
								<span><img src="imgs/icons/post.png" /></span>
								<h3>تغيير ملق التجويد</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="tajweed_man.php" method="post" enctype="multipart/form-data">
								<h2>ملف التجويد القديم : </br>
									<?php
										$sql = "SELECT * FROM tajweed";
										$res = mysqli_query($conn,$sql);
										if(mysqli_num_rows($res) > 0){
											$file = mysqli_fetch_assoc($res);
											echo $file['tajweed_file'];
										}else{
											echo "<p style='font-size:16px;'>.لا يوجد ملف تجويد</p>";
										}
									?>
								</h2>							
								<input type="file" placeholder="صورة الخبر" name="file"/>
								<button type="submit" name="add_news">إضافة</button>
							  </form>
						</div>
					</article>					
				</div>
