...
/* checking if an admin is logged in */
	...
	if(isset($_POST['std_info'])){
		...
		if((isset($_POST['att']) and isset($_POST['score1']) and isset($_POST['score2'])) or $canSkipCheck){
			...
			$sql = "INSERT INTO `std_att`(`std_id`, `date`, `state`) VALUES ('".$_GET['std_id']."','".date('Y-m-d')."','".$att."')";
			if(mysqli_query($conn,$sql)){
				$sql = "INSERT INTO `recite`(`std_id`, `date`, `grade`) VALUES ('".$_GET['std_id']."','".date('Y-m-d')."','".$score1."')";
				if(mysqli_query($conn,$sql)){
					$sql = "INSERT INTO `review`(`std_id`, `date`, `grade`) VALUES ('".$_GET['std_id']."','".date('Y-m-d')."','".$score2."')";
					if(mysqli_query($conn,$sql)){
						...
						if(mysqli_query($conn,$sql)){
							...
							if($_POST['div'] == "ay"){
								if(isset($_POST['courses'])){
									...
									for ($i = 0; $i < count($courses); $i++) {
										...
										// $nu = $conn->real_escape_string($numberSelector[$i]);
										// $qu = $conn->real_escape_string($quarterHalfSelector[$i]);
										...
										if($course != "none"){
											...
											$sql = "INSERT INTO `std_homework_soras`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`type`) VALUES ('".$_GET['std_id']."','".$course."','".$page1_range."','".$page2_range."','".date('Y-m-d')."','review')";
											$result = mysqli_query($conn,$sql);
											if(!$result){
									...
									if(isset($_POST['sora'])){
										...
										for ($i = 0; $i < count($courses); $i++) {
											...
											if($course != "none"){
												...
												$sql = "INSERT INTO `std_homework_soras`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`type`) VALUES ('".$_GET['std_id']."','".$course."','-','-','".date('Y-m-d')."','review')";
												$result = mysqli_query($conn,$sql);
												if(!$result){
													echo mysqli_error($conn);
												}
							...
							if($_POST['cdiv'] == "cay"){
								if(isset($_POST['ccourses'])){
									// // $numberSelector = $_POST['cnumberSelector'];
									// // $quarterHalfSelector = $_POST['cquarterHalfSelector'];
									...
									for ($i = 0; $i < count($ccourses); $i++) {
										...
										if($course != "none" and $course != ""){
											...
											// 	//echo "Error: " . $e->getMessage(); // Handle the Exception by printing the error message
											// }
											$sql = "INSERT INTO `std_homework_soras`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`type`) VALUES ('".$_GET['std_id']."','".$course."','".$page1_range."','".$page2_range."','".date('Y-m-d')."','recite')";
									...
									if(isset($_POST['csora'])){									
										$courses = $_POST["csora"];
										// $numberSelector = $_POST['csnumberSelector'];
										// $quarterHalfSelector = $_POST['csquarterHalfSelector'];
										// Loop through the selected courses and page ranges
										for ($i = 0; $i < count($courses); $i++) {
											$course = $conn->real_escape_string($courses[$i]);
											//$nu = $conn->real_escape_string($numberSelector);
											...
											if($course != "none" and $course != ""){
												...
												// 	//echo "Error: " . $e->getMessage(); // Handle the Exception by printing the error message
												// }
												$sql = "INSERT INTO `std_homework_soras`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`type`) VALUES ('".$_GET['std_id']."','".$course."','-','-','".date('Y-m-d')."','recite')";
									...
									if(isset($_POST['cpart'])){	
										$courses = $_POST["cpart"];
										// $numberSelector = $_POST['cfnumberSelector'];
										// $quarterHalfSelector = $_POST['cfquarterHalfSelector'];
										// Loop through the selected courses and page ranges
										for ($i = 0; $i < count($courses); $i++) {
											$course = $conn->real_escape_string($courses[$i]);
								...
								if(isset($_POST["csora"])){
									...
									for ($i = 0; $i < count($courses); $i++) {
										$course = $conn->real_escape_string($courses[$i]);
										// $nu = $conn->real_escape_string($numberSelector[0]);
										// $qu = $conn->real_escape_string($quarterHalfSelector[0]);
							...
							mysqli_query($conn,"
												...
												GROUP BY std_id, sora, s_from, s_to, date, type, face;
    ...
    echo '<h1 style="font-size:18px;color:#1c9300;">الطالب ليس لديه واجب مراجعة</h1></br>';
									...
									<tr>
											...
											<td>
												...
												<input style="width:auto;margin:10px;" type="radio" name="score1" value="مقبول" id="grade4"> مقبول</br>
												<input style="width:auto;margin:10px;" type="radio" name="score1" value="ضعيف" id="grade5"> ضعيف</br>
											...
											<td>
												...
												<input style="width:auto;margin:10px;" type="radio" name="score2" value="مقبول" id="grade10"> مقبول</br>
												<input style="width:auto;margin:10px;" type="radio" name="score2" value="ضعيف" id="grade11"> ضعيف</br>
						...
						<article class="card" id="homework" style="margin-top:10px">
							...
							<div class="card-body">
								...
								<center>
									...
									<input style="width:auto;margin-left:10px;" type="radio" name="div" id="div1-radio" value="ay" onclick="showDiv('div1')" checked>
												...
												<ul>
													...
													<li style="display:inline;">
														...
														<div id="optionsContainer" style="display:inline-block;width:20%;">
															...
															<div id="quarterHalfSelection"></div>
														</div>
													</li>
												</ul>
											</div>
									...
									<div id="pa" style="margin-bottom:10px;">
										<div class="part" id="part1">
											<ul>
												...
												<li style="display:inline;">
														<div id="fnumberSelectorContainer" style="width:20%;display:inline-block;">
															<select id="fnumberSelector" name="fnumberSelector" style="width:100%;height:30px;">
																<option value="" style="padding:5px;">عدد الأوجه</option>
																<option value="1">1</option>
																<option value="2">2</option>
																<option value="3">3</option>
														...
														<div id="foptionsContainer" style="display:inline-block;width:20%;">
															<select id="fquarterHalfSelector" name="fquarterHalfSelector" style="width:100%;height:30px;">
															<option value="0" selected>-----------</option>
															<option value="ربع">ربع</option>
															<option value="نصف">نصف</option>
															</select>
									...
									<div id="ccourse-container" style="margin-bottom:10px;">
										<div class="ccourse-container" id="ccourse1-container">
											<ul>
												...
												<li style="display:inline;">
														<div id="cnumberSelectorContainer" style="width:20%;display:inline-block;">
															<select id="cnumberSelector" name="cnumberSelector[]" style="width:100%;height:30px;">
																<option value="" style="padding:5px;">عدد الأوجه</option>
																<option value="1">1</option>
																<option value="2">2</option>
																<option value="3">3</option>
															...
															<div id="cnumberSelection"></div>
														</div>
														<div id="coptionsContainer" style="display:inline-block;width:20%;">
															<select id="cquarterHalfSelector" name="cquarterHalfSelector[]" style="width:100%;height:30px;">
															<option value="0" selected>-----------</option>
															<option value="ربع">ربع</option>
															<option value="نصف">نصف</option>
															</select>
															<div id="cquarterHalfSelection"></div>
														</div>
													</li>
											</ul>
										</div>
									...
									<div id="csora" style="margin-bottom:10px;">
										<div class="csora" id="csora1">
											<ul>
												...
												<li style="display:inline;">
														<div id="csnumberSelectorContainer" style="width:20%;display:inline-block;">
															<select id="csnumberSelector" name="csnumberSelector" style="width:100%;height:30px;">
																<option value="" style="padding:5px;">عدد الأوجه</option>
																<option value="1">1</option>
																<option value="2">2</option>
																<option value="3">3</option>
														...
														<div id="csoptionsContainer" style="display:inline-block;width:20%;">
															<select id="csquarterHalfSelector" name="csquarterHalfSelector" style="width:100%;height:30px;">
															<option value="0" selected>-----------</option>
															<option value="ربع">ربع</option>
															<option value="نصف">نصف</option>
															</select>
															<div id="csquarterHalfSelection"></div>
														</div>
													</li>
											</ul>
										</div>
								...
								<center><div id="cdiv3" class="ccont" style="display:none;">
									<div id="cpa" style="margin-bottom:10px;">
										<div class="cpart" id="cpart1">
											<ul>
												<li style="display:inline;">
													<select style="display:flex;width:30%;height:80px;margin-bottom:8px;" name="cpart[]" id="p1" multiple>
												...
												<li style="display:inline;">
														<div id="cfnumberSelectorContainer" style="width:20%;display:inline-block;">
															<select id="cfnumberSelector" name="cfnumberSelector" style="width:100%;height:30px;">
																<option value="" style="padding:5px;">عدد الأوجه</option>
																<option value="1">1</option>
																<option value="2">2</option>
																<option value="3">3</option>
														...
														<div id="cfoptionsContainer" style="display:inline-block;width:20%;">
															<select id="cfquarterHalfSelector" name="cfquarterHalfSelector" style="width:100%;height:30px;">
															<option value="0" selected>-----------</option>
															<option value="ربع">ربع</option>
															<option value="نصف">نصف</option>
															</select>
...
<script>
	...
	<script>
		...
		function validateForm() {
			...
			const selectedNumber = document.getElementById("numberSelector").value;
			const selectedQuarterHalf = document.getElementById("quarterHalfSelector").value;
			...
			if(rad1.checked){
				if (!selectedNumber || !selectedQuarterHalf) {
					ok = false;
				}
			}
			if(rad2.checked){
				if (!sselectedNumber || !sselectedQuarterHalf) {
					ok = false;
				}
			}
			if(crad1.checked){
				if (!cselectedNumber || !cselectedQuarterHalf) {
					ok = false;
				}
			}
			if(crad2.checked){
				if (!csselectedNumber || !cselectedQuarterHalf) {
					ok = false;
				}
			}
			...
			if (ok) {
				...
				alert("الرجاء اختيار عدد الأوجه.");
				return false;
			}
