...
/* move std to other ring */
	...
	if(isset($_POST['move_std'])){
		$admins = $_POST['admins'];
		mysqli_query($conn,"DELETE FROM ring_admins WHERE ring_id = '".$_GET['ring_id']."'");
		if(isset($_POST['admins'])){
			$admins = $_POST['admins'];
			if (!empty($admins)) {
				// Combnine the teacher with their ring
				foreach ($admins as $ad_id) {
					$sql = "INSERT INTO ring_admins (ring_id, staff_id) VALUES ('".$_GET['ring_id']."','".$ad_id."')";
					mysqli_query($conn,$sql);
				}				
			}
		}
		else{
			echo "<script>alert('حدث خطأ في إضافة الإداري')</script>";
		}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إضافة مشرف</title>
					...
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>تعديل أو حذف مشرف</h3>
							</div>
						</div>
						<div class="card-body">
						<form class="login-form" action="ring_add_supervisor.php?ring_id=<?php	echo $_GET['ring_id']; ?>" method="post" onsubmit="return validateForm();">
								...
								<ul>
									...
									if(mysqli_num_rows($result) > 0) {
										while($row = mysqli_fetch_assoc($result)) {
											$re = mysqli_query($conn,"SELECT staff_id FROM ring_admins WHERE ring_id = '".$_GET['ring_id']."' AND staff_id = '".$row['staff_id']."'");
											if(mysqli_num_rows($re) > 0){
												echo '<li><input style="width:auto;margin:10px;" type="checkbox" name="admins[]" value="'. $row["staff_id"] .'" checked>'. $row["staff_name"] .'</li>';
											}else{
							...
							<button type="submit" name="move_std">تعديل</button>
						</form>

						<script>
							function validateForm() {
								var selectBox = document.getElementById("staff-select");
								if (selectBox.value === "") {
									alert("يجب عليك اختيار خيار");
