...
if((isset($_SESSION['email'])!= "") && (isset($_SESSION['staff_job']) == "JOB_01")) {
	...
	if(isset($_POST['delete_news'])){
		$sql = "DELETE FROM `main_posts` WHERE post_id = '".$_GET['id']."'";
		$result = mysqli_query($conn,$sql);
		if(!$result){
			echo "<script>alert('خطأ في حذف الخبر')</script>";
		}else{
			echo "<script>window.location.href='main_news_man.php';</script>";
		}
	}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إدارة الصفحة الرئيسية</title>
	...
	<div class="responsive-wrapper">
		...
		<div class="content-header">
			...
			<div class="content-header-actions">
				...
				<a href="new_main_post.php" class="button">
							...
							echo '<article class="card">
									<div class="card-header">
										<div>
											...
											<h3 id="test">'.$row['title'].'</h3>
										</div>

									</div>
									<div class="card-body">
									<div><p class="limit-text"> تفاصيل الخبر : '.$row['secondary_title'].'</p></div>
										<p> تاريخ النشر : '.$row['date'].'</p>
										<p> تاريخ الانتهاء : '.$row['end_date'].'</p>
									</div>
									<div class="card-footer">
										<form class="login-form" action="main_news_man.php?id='.$row['post_id'].'" method="post">
											<button type="submit" name="delete_news" onclick="return confirmDelete()">حذف الخبر</button>
										</form>
									</div>
								</article>
							';
  ...
  <script>
	...
	<script>
		...
		link.addEventListener('click', function(event) {
			...
			textContainer.classList.toggle('limit-text');
			this.textContent = textContainer.classList.contains('limit-text') ? 'Read more' : 'Read less';
		});
		});
	</script>
</body>
