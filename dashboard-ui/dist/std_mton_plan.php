		...
		<div class="content">
			...
			<div class="content-main">
				...
				<div class="card-grid">
					...
					<?php
						$sql = "SELECT students.std_name,students.std_id,mton_ring.mton_ring_id FROM mton_ring 
								LEFT JOIN students ON mton_ring.mton_ring_id = students.mton_ring_temp_id
								WHERE students.mton_ring_temp_id = '".$_GET['mton_ring_id']."' OR students.mton_ring_temp_id_02 = '".$_GET['mton_ring_id']."' OR students.mton_ring_temp_id_03 = '".$_GET['mton_ring_id']."'";
						$result = mysqli_query($conn,$sql);
						if($result){
							
							while($row = mysqli_fetch_assoc($result)){
								
								$sql = "SELECT * FROM mton_review_plan WHERE std_id = '".$row['std_id']."'";
								echo '
									<tr>
								...
								if(mysqli_num_rows($r) > 0){
									...
									//if($rr['type'] == '2'){
										echo '<td><ul>';
										$sql = "SELECT * FROM mton_review_plan
												INNER JOIN mton ON mton.mton_id = mton_review_plan.sora
										 		WHERE std_id = '".$row['std_id']."'";
										$ress = mysqli_query($conn,$sql);
										while($rrr = mysqli_fetch_assoc($ress)){
											echo '<li>'.$rrr['mton_name'].'</li>';
											echo '<li> عدد الأبيات : '.$rrr['amount'].'</li>';
										}
										echo '<li>
												<form style = "margin-top:20px;" class="login-form" action="new_mton_review_plan.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'" method="post">
									...
									echo '<td>
											<form class="login-form" action="new_mton_review_plan.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'" method="post">
												<button type="submit" name="te">إضافة</button>
											</form>
										</td>';
								}
								// خطة المراجعة
								$sql = "SELECT * FROM mton_recite_plan WHERE std_id = '".$row['std_id']."'";
								...
								if(mysqli_num_rows($r) > 0){
									...
									//if($rr['type'] == '2'){
										echo '<td><ul>';
										$sql = "SELECT * FROM mton_recite_plan
												INNER JOIN mton ON mton.mton_id = mton_recite_plan.sora
										 		WHERE std_id = '".$row['std_id']."'";
										$ress = mysqli_query($conn,$sql);
										while($rrr = mysqli_fetch_assoc($ress)){
											echo '<li> المتون المراد حفظها : '.$rrr['mton_name'].'</li>';
											echo '<li> عدد الأبيات : '.$rrr['amount'].'</li>';
										}
										echo '<li>
												<form style = "margin-top:20px;" class="login-form" action="new_mton_recite_plan.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'" method="post">
													<button type="submit" name="te">تعديل</button>
												</form>
											</li>
									</ul></td>';
									...
									echo '<td>
											<form class="login-form" action="new_mton_recite_plan.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'" method="post">
												<button type="submit" name="add_reviwe_plan">إضافة</button>
											</form>
										</td>';
								}
								echo'	</tr>
