...
if((isset($_SESSION['email'])!= "") && (isset($_SESSION['staff_job']) == "JOB_01")) {
	...
	if(isset($_GET['std_id'])){
		$sql = "UPDATE students SET state = 'منتظم' WHERE std_id = '".$_GET['std_id']."'";
		if(mysqli_query($conn,$sql)){
			echo "<script>alert('تم استرجاع الطالب');</script>";
			echo "<script>window.location.href='std_management_fired.php';</script>";
		}
	}
?>
<!DOCTYPE html>
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>أرشيف الطلاب المفصولين</title>
	...
	<div class="responsive-wrapper">
		...
		<div class="content-header">
			<div class="content-header-intro">
				<h2 >أرشيف الطلاب المفصولين</h2>
				<?php if($_SESSION['staff_job'] == 'JOB_01'){ ?>
				<center><h3 style="font-weight:bold;font-size:24px;">عدد الطلاب في الأرشيف : <color style="color:#ffffff;"><?php 
		...
		<div class="content">
			...
			<center style="margin-bottom:10px;"><div class="content-header-actions">
				<a href="std_info_fired.php" class="button">
					<span>سجل الطلاب</span>
				</a>
			</div></center>
				
				<center style="margin-bottom:10px;"><div class="search">
					<form action="std_management_fired.php?search=1" methode="get">
					...
					<?php
						if(isset($_GET['search'])){
							/*$sql = "SELECT * FROM students
									...
									WHERE ring.ring_man = '".$_SESSION['email']."' AND students.state = 'مفصول' AND students.std_name LIKE '%".$_GET['search']."%'";
									$sql = "SELECT * FROM students
									WHERE students.std_name LIKE '%".$_GET['search']."%'";*/
									//if($_SESSION['staff_job'] == 'JOB_01'){
										$sql = "SELECT * FROM students
												WHERE students.state = 'مفصول' AND students.std_name LIKE '%".$_GET['search']."%'";
									//}
							...
							while($row = mysqli_fetch_assoc($result)){
								echo '<article class="card">
										...
										<div class="card-footer">
											<a href="std_management_fired.php?std_id='.$row['std_id'].'" onclick="return confRet();">استرجاع الطالب</a>
										</div>';
										echo '<div class="card-footer">
												<a href="std_more_details_fired.php?id='.$row['std_id'].'">مزيد من التفاصيل</a>
											</div>';
							...
							$sql = "SELECT * FROM students
									INNER JOIN ring ON ring.ring_id = students.ring_id
									INNER JOIN staff ON staff.staff_id = ring.ring_man
									WHERE students.state = 'مفصول' AND ring.ring_man = '".$_SESSION['email']."' ORDER BY students.std_name";
									if($_SESSION['staff_job'] == 'JOB_01'){
										$sql = "SELECT * FROM students WHERE students.state = 'مفصول' ORDER BY students.std_name";
									}
							$result = mysqli_query($conn,$sql);
							while($row = mysqli_fetch_assoc($result)){
								echo '<article class="card">
...
<script>
		...
		function confRet() {
			if (confirm("هل أنت متأكد من استرجاع الطالب من الأرشيف ؟")) {
				return true;
			} else {
				return false;
			}
