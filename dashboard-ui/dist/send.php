...
if((isset($_SESSION['email'])!= "") && (isset($_SESSION['staff_job']) == "JOB_01")) {
	...
	if(isset($_POST['submit'])){
		$msg = $_POST['msg'];
		$send = $_POST['send'];
		$sql = "INSERT INTO `messages`(`user_id`, `user_type`, `msg`, `date`) VALUES ('".$send."','".$_GET['type']."','".$msg."','".date('Y-m-d')."')";
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إرسال الرسالة</title>
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="send.php?type=<?php echo $_GET['type'];?>" method="post">
								<select name="send" id="staff-select">
									<?php
										if($_GET['type'] == 0){
											$sql = "SELECT * FROM staff WHERE staff_job != 'JOB_01'";
										...
										if($_GET['type'] == 1){
											...
											while($row = mysqli_fetch_assoc($result)){
												echo '
													<option value="'.$row['std_id'].'" selected>'.$row['std_name'].'</option>
												';
											}
										}
										if($_GET['type'] == 2){
											$sql = "SELECT * FROM parent WHERE p_id != 13";
											$result = mysqli_query($conn,$sql);
											while($row = mysqli_fetch_assoc($result)){
												echo '
													<option value="'.$row['p_id'].'" selected>'.$row['p_name'].'</option>
												';
											}
										}
