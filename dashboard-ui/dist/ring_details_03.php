	...
	if(isset($_POST['std_info'])){
		...
		if(isset($_POST['att']) or $canSkipCheck){
			...
			if(mysqli_query($conn,$sql)){
				$sql = "INSERT INTO `recite`(`std_id`, `date`, `grade`) VALUES ('".$_GET['std_id']."','".date('Y-m-d')."','".$score2."')";
				if(mysqli_query($conn,$sql)){
					$sql = "INSERT INTO `review`(`std_id`, `date`, `grade`) VALUES ('".$_GET['std_id']."','".date('Y-m-d')."','".$score1."')";
					if(mysqli_query($conn,$sql)){
						...
						if(mysqli_query($conn,$sql)){
							...
							include('sorahs.php');
							if($_POST['div'] == "no"){
								$sql = "SELECT date FROM std_homework_soras WHERE sora != '' AND sora != ' ' AND std_id = '".$_GET['std_id']."' AND type ='review'
										ORDER BY DATE DESC
										LIMIT 1";
								$ddate = mysqli_fetch_assoc(mysqli_query($conn,$sql))['date'];

								$sql = "SELECT * FROM std_homework_soras WHERE std_id = '".$_GET['std_id']."' AND date = '".$ddate."' AND type ='review'";
								$re = mysqli_query($conn,$sql);
								if(mysqli_num_rows($re) > 0){
									while($row2 = mysqli_fetch_assoc($re)){
										mysqli_query($conn,"
											INSERT INTO `std_homework_soras`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`type`) VALUES ('".$_GET['std_id']."','".$row2['sora']."','".$row2['s_from']."','".$row2['s_to']."','".date('Y-m-d')."','".$row2['type']."')
										");
									}
								}
							}
								...
								if(isset($_POST['courses'])){
									...
									if(isset($_POST['numberSelector'])){
										...
										$sql = "INSERT INTO `sora_face`(`std_id`, `face`, `type`, `date`) VALUES ('".$_GET['std_id']."','".$val."','review','".date('Y-m-d')."')";
										mysqli_query($conn,$sql);
									}
							...
							if($_POST['cdiv'] == "cno"){
								...
								$sql33 = "SELECT date FROM std_homework_soras WHERE sora != '' AND sora != ' ' AND std_id = '".$_GET['std_id']."' AND type ='recite'
											ORDER BY DATE DESC
											LIMIT 1";
								$ddate33 = mysqli_fetch_assoc(mysqli_query($conn,$sql33))['date'];

								$sql = "SELECT * FROM std_homework_soras WHERE std_id = '".$_GET['std_id']."' AND date = '".$ddate33."' AND type ='recite'";
								$re = mysqli_query($conn,$sql);
								if(mysqli_num_rows($re) > 0){
									while($row22 = mysqli_fetch_assoc($re)){
										mysqli_query($conn,"
											INSERT INTO `std_homework_soras`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`type`) VALUES ('".$_GET['std_id']."','".$row22['sora']."','".$row22['s_from']."','".$row22['s_to']."','".date('Y-m-d')."','".$row22['type']."')
										");
									}
								}
								...
								if(isset($_POST['ccourses'])){
									...
									if(isset($_POST['snumberSelector'])){
										...
										$sql = "INSERT INTO `sora_face`(`std_id`, `face`, `type`, `date`) VALUES ('".$_GET['std_id']."','".$val."','recite','".date('Y-m-d')."')";
										mysqli_query($conn,$sql);
									}
