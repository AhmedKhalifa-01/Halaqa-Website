		...
		<div class="content">
			...
			<div class="content-main">
				...
				<center>
					...
					<?php
						$sql = "SELECT students.std_name,students.std_id,ring.ring_id,students.temp_ring_id FROM ring 
						...
						if($result){
							while($row = mysqli_fetch_assoc($result)){
								$sql = "SELECT * FROM review_plan WHERE std_id = '".$row['std_id']."'";
								echo '
									<tr>
										<td style="white-space: normal;word-wrap: break-word;width: 50%;">'.$row['std_name'].'</td>';
								...
								if(mysqli_num_rows($r) > 0){
									...
									if($rr['type'] == '3'){
										echo '<td style="width:100px;">
												<ul>
													...
													<li>
														<form style = "margin-top:20px;" class="login-form" action="new_review_plan.php?std_id='.$row['std_id'].'&ring_id='.$_GET['ring_id'].'" method="post">
									...
									echo '<td>
											<form class="login-form" action="new_review_plan.php?std_id='.$row['std_id'].'&ring_id='.$_GET['ring_id'].'" method="post">
												<button type="submit" name="te">إضافة</button>
											</form>
										</td>';
								}
								// خطة المراجعة
								$sql = "SELECT * FROM recite_plan WHERE std_id = '".$row['std_id']."'";
								...
								if(mysqli_num_rows($r) > 0){
									...
									if($rr['type'] == '3'){
										echo '<td>
												<ul>
													...
													<li>
														<form style = "margin-top:20px;" class="login-form" action="new_recite_plan.php?std_id='.$row['std_id'].'&ring_id='.$_GET['ring_id'].'" method="post">
															<button type="submit" name="add_reviwe_plan">تعديل</button>
														</form>
													</li>
												</ul>
											</td>';
									...
									echo '<td>
											<form class="login-form" action="new_recite_plan.php?std_id='.$row['std_id'].'&ring_id='.$_GET['ring_id'].'" method="post">
