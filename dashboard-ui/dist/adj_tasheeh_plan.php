...
/* adding new review plan */
	...
	if(isset($_POST['t_plan'])){
			if($_POST['amount'] != ""){
				$amount = $_POST['amount'];
				$sql = "DELETE FROM tasheeh_plan WHERE tp_std_id = '".$_GET['std_id']."'";
				mysqli_query($conn,$sql);
				$sql = "INSERT INTO `tasheeh_plan`(`tp_std_id`, `tp_plan`) VALUES ('".$_GET['std_id']."','".$amount."')";
				if(mysqli_query($conn,$sql)){
					echo "<script>window.location.href='tasheeh_plan.php?t_id=".$_GET['t_id']."';</script>";
				}else{
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>خطة التصحيح</title>
				...
				<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>إضافة خطة تصحيح</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="adj_tasheeh_plan.php?std_id=<?php echo $_GET['std_id']; ?>&t_id=<?php echo $_GET['t_id']; ?>" method="post">
								
								<div id="face">
									<p>يمكن كتابتها نصا، مثل : وجه  نصف </p>
									<input style="width:100%;" type="text" placeholder = "عدد الأوجه" name="amount">
								</div>		
								<button type="submit" name="t_plan">إضافة الخطة</button>					
							  </form>
						</div>
					</article>	
