...
<form action="my_msgs.php" method="post" style="margin-right:100px;">		
		<center><div style="display:position:fixed;margin-auto;width:100%;">
			...
			<table>
				...
				<?php
						...
						if($_SESSION['staff_job'] == 'JOB_03'){
							$sql = "SELECT * FROM messages WHERE user_id = '".$_SESSION['email']."' AND user_type = 0 ORDER BY msg_id DESC";
							mysqli_query($conn,"UPDATE messages SET seen = 1 WHERE user_id = '".$_SESSION['email']."' AND user_type = 0");
						}
						if($_SESSION['staff_job'] == 'JOB_02'){
							$sql = "SELECT * FROM messages WHERE user_id = '".$_SESSION['email']."' AND user_type = 3 ORDER BY msg_id DESC";
							mysqli_query($conn,"UPDATE messages SET seen = 1 WHERE user_id = '".$_SESSION['email']."' AND user_type = 3");
						}
						if($_SESSION['staff_job'] == 'student'){
							$sql = "SELECT * FROM messages WHERE user_id = '".$_SESSION['email']."' AND user_type = 1 ORDER BY msg_id DESC";
							mysqli_query($conn,"UPDATE messages SET seen = 1 WHERE user_id = '".$_SESSION['email']."' AND user_type = 1");
						}
						$result = mysqli_query($conn,$sql);
						if($result){
							while($row = mysqli_fetch_assoc($result)){
