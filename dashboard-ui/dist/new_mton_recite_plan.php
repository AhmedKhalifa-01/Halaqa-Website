...
/* adding new review plan */
	...
	if(isset($_POST['amrp'])){
			if(isset($_POST['mton'])){
				$mton = $_POST['mton'];
				$amount = $_POST['amount'];
				$sql = "DELETE FROM mton_recite_plan WHERE std_id = '".$_GET['std_id']."'";
				mysqli_query($conn,$sql);
				$sql = "INSERT INTO `mton_recite_plan`(`std_id`,`amount`,`sora`) VALUES ('".$_GET['std_id']."','".$amount."','".$mton."')";
				if(mysqli_query($conn,$sql)){
					echo "<script>window.location.href='std_mton_plan.php?mton_ring_id=".$_GET['mton_ring_id']."';</script>";
				}
			}else{
				echo "<script>alert('الرجاء اختيار المتون');</script>";
			}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إضافة خطة مراجعة </title>
				...
				<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>إضافة خطة مراجعة</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="new_mton_recite_plan.php?std_id=<?php echo $_GET['std_id']; ?>&mton_ring_id=<?php echo $_GET['mton_ring_id']; ?>" method="post">
								<div id = "sorah">
									<select style = "padding:10px;width:100%;height:50px;;font-size:18px;" name="mton" >
										<?php
											$sql = "SELECT * FROM mton";
											$res = mysqli_query($conn,$sql);
											while ($sa = mysqli_fetch_assoc($res)) {
												echo '<option value="'.$sa['mton_id'].'">'.$sa['mton_name'].'</option>';
											}
										?>
									</select>
									<input style="width:50%;" type="text" placeholder = "عدد الأوجه" name="amount">
								</div>
								<button type="submit" name="amrp">إضافة الخطة</button>					
							  </form>
						</div>
					</article>	
