	...
	if(isset($_POST['del'])){
		if(isset($_POST['students'])){
			...
			foreach ($students as $std_id) {
				$sql = "DELETE FROM prize_participating_students WHERE std_id = '".$std_id."' AND prize_id='".$_GET['prize_id']."'";
				$result = mysqli_query($conn,$sql);
				if(!$result){
					echo "<script>alert('خطأ في الحذف من للجائزة')</script>";
					echo "<script>window.location.href='prize_details.php?prize_id=".$_GET['prize_id']."';</script>";
				}else{
					echo "<script>alert('تم الحذف من الجائزة')</script>";
					echo '<script>window.location.href="prize_details.php?&prize_id='.$_GET['prize_id'].'";</script>';
				}
			}
		}
...
<main class="main">
	<div class="responsive-wrapper">
		...
		<div class="content">
			...
			<a href="prize_del_std.php?prize_id=<?php echo $_GET['prize_id']; ?>" class="button" style="margin-left:10px;width:100px;height:50px;">
					<span>رجوع</span>
				</a></br></br>
			</center>
					...
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>حذف طلبة من الجائزة</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="prize_del_select_std.php?prize_id=<?php echo $_GET['prize_id']; ?>" method="post">
								<p>اختر الطلبة لحذفهم من الجائزة : </p>
								<ul style="text-align:right;">
									...
									$sql = "SELECT *
											...
											WHERE students.ring_id = '".$_GET['ring_id']."' AND prize_id = '".$_GET['prize_id']."'
									...
									if(mysqli_num_rows($result) > 0) {
										...
										echo '<button type="submit" name="del">حذف</button>';
									} else {
										echo "لا يوجد طلبة لحذفهم.";
									}
									?>
								</ul>
