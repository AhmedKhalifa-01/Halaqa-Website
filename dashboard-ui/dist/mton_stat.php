...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إحصائية المتون</title>
	...
	<div class="responsive-wrapper">
			...
			<div class="content-header-actions">
				<?php if(isset($_GET['hide'])){?>
					<div class="content-header-actions">
						...
						<a href="mton_stat.php?mton_id=<?php echo $_GET['mton_id']; ?>" class="button">
							<span>الرجوع</span>
						</a>
					</div>
				<?php } ?>
		...
		<div class="content-panel">
				<div class="vertical-tabs">
					...
					<a href="parents_man.php">إدارة أولياء الأمور</a>
					<?php } ?>
						<a href="mton_man.php">إدارة الحلقات  التسميع</a>
			...
			<?php if(isset($_GET['ring_id'])) {?>	
				<?php if(!isset($_GET['hide'])){?>
					<article class="card" style="width:300px;">
							<div class="card-header">
								<div>
									<span><img src="imgs/icons/quran.png" /></span>
									<h3>احصائية الحلقة بين فترتين</h3>
								</div>
							</div>
							
							<div class="card-body">	
								<form class="login-form" action = "mton_stat_table.php?ring_id=<?php echo $_GET['ring_id']; ?>" method = "post" target="_blank">
				...
				<?php }else{?>
					...
					$sql = "SELECT ring.ring_id,ring.ring_name,t.staff_name AS teacher_name, s.staff_name AS supervisor_name, COUNT(students.std_id) AS student_count
							...
							$sql = "SELECT * FROM ring 
									...
									WHERE ring.staff_id = '".$_SESSION['email']."' GROUP BY ring.staff_id";
						}
							...
							while($row = mysqli_fetch_assoc($result)){
								echo '<article class="card">
										...
										echo '</div><div class="card-footer">
													<a style= "color:red;" href="mton_stat.php?ring_id='.$row['ring_id'].'">عرض الإحصائيات</a>
												</div>';
										echo '
									</article>
								';
