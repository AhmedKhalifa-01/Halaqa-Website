	...
	if(isset($_POST['std_info'])){
		...
		if(isset($_POST['att']) or $canSkipCheck){
			...
			if(mysqli_query($conn,$sql)){
				...
				$sql = "INSERT INTO `std_att`(`std_id`, `date`, `state`, `rev_id`, `time`) VALUES ('".$_GET['std_id']."','".date('Y-m-d')."','".$att."','".$last_inserted_id."','".$current_time."')";
				if(mysqli_query($conn,$sql)){
								...
								if(mysqli_num_rows($pRes) > 0){
									...
									while($rowP = mysqli_fetch_assoc($resultP)){
										...
										if($rowP['star'] == 'البرونزي'){
											$hasPronze = true;
											if($rowP['total'] >= 2){
												$sql = "INSERT INTO `prize_details`(`std_id`, `date`, `star`,`prize_id`) VALUES ('".$_GET['std_id']."','".date('Y-m-d')."','البرونزي','".$rowPrize['prize_id']."')";
												$result = mysqli_query($conn,$sql);
												if($rowP['total'] % 3 == 0){
													$sql = "INSERT INTO `prize_details`(`std_id`, `date`, `star`,`prize_id`) VALUES ('".$_GET['std_id']."','".date('Y-m-d')."','الفضي','".$rowPrize['prize_id']."')";
													$result = mysqli_query($conn,$sql);
													if($result){
														$sql = "DELETE FROM `prize_details` WHERE std_id = '".$_GET['std_id']."' AND star = 'البرونزي'";
														mysqli_query($conn,$sql);
														$sql = "SELECT star,COUNT(std_id) As total FROM prize_details WHERE prize_details.std_id = '".$_GET['std_id']."' AND star='الفضي' GROUP BY star ";
														$result = mysqli_query($conn,$sql);
														if(mysqli_num_rows($result) > 0){
															$row = mysqli_fetch_assoc($result);
															if($row['total'] > 2){
																if($row['total'] % 3 == 0){
																	$sql = "INSERT INTO `prize_details`(`std_id`, `date`, `star`,`prize_id`) VALUES ('".$_GET['std_id']."','".date('Y-m-d')."','الذهبي','".$rowPrize['prize_id']."')";
																	mysqli_query($conn,$sql);
																}
																$sql = "DELETE FROM `prize_details` WHERE std_id = '".$_GET['std_id']."' AND star = 'الفضي'";
																mysqli_query($conn,$sql);
															}
														}
													}
