...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إدارة المجمعات</title>
...
<header class="header">
	...
	<div class="header-logo">
			...
			<?php if($_SESSION['staff_job'] != "JOB_03"){?>
					...
					<a href="tasheeh_man.php">مجمع تصحيح التلاوة</a>
					...
					<?php
						if($_SESSION['staff_job'] == 'JOB_01'){
							...
							echo '<a href=" community_man.php"  class="active">إدارة المجمعات</a>';
						}else{
	...
	<div class="responsive-wrapper">
		...
		<div class="content-header">
			...
			<div class="content-header-intro">
				<h2>إدارة المجمعات</h2>
				<?php if($_SESSION['staff_job'] == 'JOB_01'){ ?>
				<center><h3 style="font-weight:bold;font-size:24px;">عدد المجمعات : <color style="color:#ffffff;"><?php 
														$res = mysqli_query($conn,"SELECT COUNT(id) AS count FROM community"); 
			...
			<div class="content-header-actions">
				...
				<?php if($_SESSION['staff_job'] == 'JOB_01'){?>
				<a href="new_community.php" class="button">
					<i class="ph-plus-bold"></i>
					<span>إضافة مجمع جديد  </span>
				</a>
				<a href="trans_std_community.php" class="button">
					<i class="ph-plus-bold"></i>
					<span>تحويل الطلبة بين المجمعات</span>
				</a>
				<?php } ?>
			</div>
			<div class="search" style= "margin-top:10px;">
					<form action="ring_man.php?search=1" methode="get">
						<input type="text" placeholder="اكتب اسم المجمع" name = "search"/>
						<button type="submit">
		...
		<div class="content">
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<?php 
						$sql = "SELECT name,community.id AS c_id FROM community
								";
						...
						while($row = mysqli_fetch_assoc($result)){
							// the following sql gets the number of rings in the community 
							/*$sql2 = "SELECT COUNT(ring.ring_id) AS ring_c FROM community
									 LEFT JOIN ring ON ring.community_id = community.id
									 WHERE community.id = '".$row['c_id']."'
							...
							echo '<article class="card">
									<div class="card-header">
										<div>
											...
											<h3> اسم المجمع : '.$row['name'].'</h3>
										</div>
									</div>
									<div class="card-body">
										<!--<h2>عدد الطلبة : '.$row['std_c'].'</h2>
										<h2>عدد الحلقات : '.mysqli_fetch_assoc(mysqli_query($conn,$sql2))['ring_c'].'</h2>-->
									</div>
...
<script>
		...
		function confirmDel() {
			if (confirm("هل أنت متأكد من حذف المجمع ؟")) {
				return true;
			} else {
				return false;
			}
