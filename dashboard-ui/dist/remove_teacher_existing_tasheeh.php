		...
		if(isset($_POST['teachers'])){
			...
			if (!empty($teachers)) {
				foreach ($teachers as $teacher_id) {
					$sql = "DELETE FROM tasheeh_teacher WHERE teacher_id = '".$teacher_id."' AND tasheeh_id = '".$_GET['t_id']."'";
					mysqli_query($conn,$sql);
				}				
			}
		}else{
			echo "<script>alert('الرجاء اختيار معلم واحد على الأقل')</script>";
		}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>حذف معلم من الحلقة</title>
					...
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>حذف معلم من الحلقة</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="remove_teacher_existing_tasheeh.php?t_id=<?php echo $_GET['t_id']; ?>" method="post">
								<select name="staff" id="staff-select">
									<?php
										$sql = "SELECT * FROM tasheeh WHERE id = '".$_GET['t_id']."'";
										$result = mysqli_query($conn,$sql);
										while($row = mysqli_fetch_assoc($result)){
											echo '
												<option value="'.$row['id'].'" selected>'.$row['t_name'].'</option>
											';
										}
									?>
								</select>
								<p>اختر المعلمين لحذفهم للحلقة : </p>
								<ul>
									<?php
									...
									$sql = "SELECT *
											...
											WHERE staff_id IN (
												SELECT teacher_id
												FROM tasheeh_teacher
												WHERE tasheeh_id = '".$_GET['t_id']."'
											) AND staff_job = 'JOB_03'";
									$result = mysqli_query($conn, $sql);

									// Display the list of students as checkboxes
									if(mysqli_num_rows($result) > 0) {
										while($row = mysqli_fetch_assoc($result)) {
											echo '<li><input style="width:auto;margin:10px;" type="checkbox" name="teachers[]" value="'. $row["staff_id"] .'">'. $row["staff_name"] .'</li>';
										}
										echo '<button type="submit" name="add_ring">حذف</button>';
									} else {
										echo "لا يوجد معلمين لحذفهم.";
									}
									?>
								</ul>
