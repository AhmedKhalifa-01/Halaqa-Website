...
if((isset($_SESSION['email'])!= "") && (isset($_SESSION['staff_job']) == "JOB_01")) {
	...
	if(isset($_GET['p_id'])){
		$sql = "DELETE FROM parent WHERE p_id = '".$_GET['p_id']."'";
...
<header class="header">
	...
	<div class="header-logo">
			...
			<a href="new_acc.php">إضافة حساب</a>
					...
					?>
					<a href="parents_man.php" class="active" style="background-color:darkred; color:white;">إدارة أولياء الأمور</a>
					<?php }else{?>
				...
				<div class="card-grid">
					<?php
							...
							if($_SESSION['staff_job'] == "JOB_02"){
								$sql = "SELECT DISTINCT parent.p_id,parent.p_name,parent.p_id_number,parent.email,parent.p_phone FROM parent
										INNER JOIN std_parent_rel ON parent.p_id = std_parent_rel.parent_id
										INNER JOIN students ON students.std_id = std_parent_rel.std_id
										...
										ORDER BY parent.p_name";
							}
							if(isset($_GET['search'])){
								$sql = "SELECT * FROM parent WHERE p_id != 13 AND p_name LIKE '%".$_GET['search']."%' ORDER BY parent.p_name";
								if($_SESSION['staff_job'] == "JOB_02"){
									$sql = "SELECT DISTINCT parent.p_id,parent.p_name,parent.p_id_number,parent.email,parent.p_phone FROM parent
										INNER JOIN std_parent_rel ON parent.p_id = std_parent_rel.parent_id
										INNER JOIN students ON students.std_id = std_parent_rel.std_id
										INNER JOIN ring on ring.ring_id = students.ring_id
										INNER JOIN ring_admins ON ring.ring_id = ring_admins.ring_id
										WHERE ring_admins.staff_id = '".$_SESSION['email']."' AND p_name LIKE '%".$_GET['search']."%'
										ORDER BY parent.p_name";
								}
							}
							$result = mysqli_query($conn,$sql);
							while($row = mysqli_fetch_assoc($result)){
								echo '<article class="card">
										...
										echo '<div class="info" style="display: none;">';
											echo '<p> رقم الهاتف : <color style= "color:red">'.$row['p_phone'].'</color></p>';
											if($_SESSION['staff_job'] == 'JOB_01'){
											...
											$sql = "SELECT students.std_name
													FROM parent
													INNER JOIN std_parent_rel ON std_parent_rel.parent_id = parent.p_id
													INNER JOIN students ON students.std_id = std_parent_rel.std_id
													WHERE parent.p_id = '".$row['p_id']."'
													ORDER BY std_name";
											$res = mysqli_query($conn,$sql);
											if(mysqli_num_rows($res) > 0){
												echo "<div style=\"margin-right:40px;\">";
												while($r = mysqli_fetch_assoc($res)){
													echo "<p style=\"font-size:14px;text-align:right;\">".$r['std_name']."</p></br>";
												}
												echo "</div>";
											}
										...
										echo '</div>';
											...
											if($_SESSION['staff_job'] == "JOB_01" or $_SESSION['staff_job'] == "JOB_02"){
												$sql = "SELECT date FROM is_p_following WHERE p_id = '".$row['p_id']."' ORDER BY date DESC";
												$result2 = mysqli_query($conn,$sql);
												if(mysqli_num_rows($result2) > 0){
													$row2 = mysqli_fetch_assoc($result2);
													echo '<p> آخر متابعة للابن : '.$row2['date'].'</p>';
													$today = date("Y-m-d");
													$date1 = new DateTime($row2['date']);
													$date2 = new DateTime($today);
													$interval = $date1->diff($date2);
													
													$numOfDays = intval($interval->format('%a days'));
													$followingState = "";
													if($numOfDays < 3){
														$followingState = "ولي الأمر متابع باستمرار";
														echo '<p> حالة متابعة ولي الأمر : '.$followingState.'</p>';
													}else if($numOfDays >= 3 and $numOfDays < 7){
														$followingState = "ولي الأمر متابع نوعا ما ";
														echo '<p> حالة متابعة ولي الأمر : '.$followingState.'</p>';
													}else if($numOfDays >= 7 and $numOfDays < 12){
														$followingState = "ولي الأمر قليل المتابعة ";
														echo '<p> حالة متابعة ولي الأمر : '.$followingState.'</p>';
													}else{
														$followingState = "ولي الأمر مهمل الطالب ";
														echo '<p style = "color:red;"> حالة متابعة ولي الأمر : '.$followingState.'</p>';
													}
													
													//echo "The difference between the two dates is: " . $interval->format('%a days');
												}else{
													$followingState = "ولي الأمر غير متابع ";
													echo '<p> حالة متابعة ولي الأمر : <color style = "color:red;">'.$followingState.'</color></p>';
												}
											}
											echo'
											
										</div>';
										if(isset($_GET['more_det'])){
											echo '<div class="card-footer">
													<a href="parents_man.php">اخفاء التفاصيل</a>
												</div>';
										}else{
											echo '<div class="card-footer">
													<a href="" id="show-more">مزيد من التفاصيل</a>
												</div>';
										}
										
										echo '<div class="card-footer">
											<a href="parent_adjust_details.php?p_id='.$row['p_id'].'">تعديل البيانات</a>
										</div>
										';
										if($_SESSION['staff_job'] == 'JOB_01'){
											echo '<div class="card-footer">
													<a href="parents_man.php?p_id='.$row['p_id'].'" onclick="return confirmDelete()">حذف الحساب</a>
												</div>';
										}
										echo '
									</article>
