...
/* adding new course */
	...
	if(isset($_POST['add_course'])){
		...
		$sql = "UPDATE `prize` SET `pronze`='".$pronze."',`silver`='".$silver."',`gold`='".$gold."' WHERE prize_id = ".$_GET['prize_id']."";
		$result = mysqli_query($conn,$sql);
		if($result){
			echo "<script>window.location.href='prize_man.php';</script>";
		}else{
			echo "<script>alert('خطأ في تعديل الجائزة ')</script>";
			echo "<script>window.location.href='prize_man.php';</script>";
		}
	}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>تغيير قيمة الجائزة</title>
					...
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>تغيير قيمة الجائزة</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="prize_change_value.php?prize_id=<?php echo $_GET['prize_id']?>" method="post" enctype="multipart/form-data">
								<?php 
									$sql = "SELECT * FROM prize WHERE prize_id = '".$_GET['prize_id']."'";
									$res = mysqli_query($conn,$sql);
									$ro = mysqli_fetch_assoc($res);

									echo '<p>قيمة النجمة البرونزية : <input type="text" value="'.$ro['pronze'].'" name="pronze"</p>';
									echo '<p>قيمة النجمة الفضية : <input type="text" value="'.$ro['silver'].'" name="silver"</p>';
									echo '<p>قيمة النجمة الذهبية : <input type="text" value="'.$ro['gold'].'" name="gold"</p>';
								?>
								<button type="submit" name="add_course">تغيير</button>
							</form>
						</div>
					</article>					
				</div>
