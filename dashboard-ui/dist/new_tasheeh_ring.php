...
/* adding new ring */
	...
	if(isset($_POST['add_ring'])){
		...
		$sql = "INSERT INTO tasheeh (t_name, t_super) VALUES ('".$ring_name."','".$ring_man."')";
		...
		if(isset($_POST['staff'])){
			...
			if (!empty($staff)) {
				...
				foreach ($staff as $staff_id) {
					$sql = "INSERT INTO tasheeh_teacher (tasheeh_id, teacher_id) VALUES ('".$ringId."','".$staff_id."')";
					mysqli_query($conn,$sql);
				}				
			}
		}	
		...
		if(isset($_POST['students'])){
			...
			if (!empty($students)) {
				...
				foreach ($students as $std) {
					$sql = "INSERT INTO tasheeh_std (tasheeh_id, std_id) VALUES ('".$ringId."','".$std."')";
					mysqli_query($conn,$sql);
				}
			}
		}
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="new_tasheeh_ring.php" method="post">
								<p>اسم الحلقة</p>
								...
								<ul>
									<?php
									// Retrieve the list of students from the database
									$sql = "SELECT * FROM students WHERE students.std_id NOT IN(SELECT std_id FROM tasheeh_std)";
									$result = mysqli_query($conn, $sql);
									echo mysqli_error($conn);
									// Display the list of students as checkboxes
									if(mysqli_num_rows($result) > 0) {
