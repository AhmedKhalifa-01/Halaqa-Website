...
<main class="main">
	<div class="responsive-wrapper">
		...
		<div class="content">
			...
			<center style="margin-bottom:20px;"><div><a href="std_my_mton_result.php" class="button" style="width:100px;height:50px;">
					<span>الرجوع</span>
				</a></div></center>
				<div class="card-grid">
				<center><table>
					...
					<?php
						$sql = "SELECT mh_date FROM mton_homework GROUP BY mh_date ORDER BY mh_date DESC LIMIT 7";
						...
						while($row = mysqli_fetch_assoc($result)){
							if(date('Y-m-d') == $row['mh_date']){
								...
								$sql = "SELECT COALESCE(mton_homework.sora,'لا توجد سورة') as sor,mh_from,mh_to,mh_date 
										FROM mton_homework 
										WHERE std_id = '".$_SESSION['email']."' AND mh_type='review' AND mh_date = '".$row['mh_date']."'
										";
								...
								$sql = "SELECT COALESCE(mton_homework.sora,'لا توجد سورة') as sor,mh_from,mh_to,mh_date 
										FROM mton_homework 
										WHERE std_id = '".$_SESSION['email']."' AND mh_type='recite' AND mh_date = '".$row['mh_date']."'
										";
								$rres = mysqli_query($conn,$sql);
								if(mysqli_num_rows($rres) > 0){
									echo '<td class="td"><ul>';
