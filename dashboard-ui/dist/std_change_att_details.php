...
if((isset($_SESSION['email'])!= "") && (isset($_SESSION['staff_job']) == "JOB_01")) {
	...
	if(isset($_POST['change'])){
		...
		if($mark > 10 or $mark < 0){
			...
			$sql = "UPDATE course_results_details SET mark = '".$mark."' WHERE course_results_details_id = '".$_GET['crd_id']."'";
			$result = mysqli_query($conn,$sql);
			if($result){
					//std_id='.$_GET['std_id'].'&c_id='.$_GET['c_id'].'
				echo "<script>window.location.href='course_std_details.php?std_id=".$_GET['std_id']."&c_id=".$_GET['c_id']."';</script>";
			}else{
				echo "<script>alert('خطأ في تعديل الدرجة !')</scipt>";
			}
		}
		...
		<div class="content">
			...
			<div class="content-main">
				...
				<table style="width:300px;">
					...
					<tbody>
					<?php
						$sql = "SELECT * FROM course_results_details INNER JOIN students ON course_results_details.std_id = students.std_id INNER JOIN course ON course_results_details.c_id = course.c_id WHERE course_results_details.std_id = '".$_GET['std_id']."' AND course_results_details.c_id = '".$_GET['c_id']."';";
						$result = mysqli_query($conn,$sql);
						if($result){
							while($row = mysqli_fetch_assoc($result)){
								echo '<tr>
										<td id = "std_name">'.$row['std_name'].'</td>
										<td>'.$row['c_name'].'</td>
										<td>'.$row['date'].'</td>
										<td>'.getDayOfWeek($row['date']).'</td>
										<td>
											<form action="std_change_att_details.php?crd_id='.$row['course_results_details_id'].'&std_id='.$_GET['std_id'].'&c_id='.$_GET['c_id'].'" method="post">
												<input type="text" placeholder="'.$row['mark'].'" name="mark" style="padding:10px; font-size:16px;width:50px;"/></br></br>
												<button type="submit" name="change" style="width:74px;">تغيير</button>
											</form>
										</td>
									</tr>
										';	
