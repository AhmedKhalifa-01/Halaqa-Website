		...
		<div class="content">
			<div class="content-panel">
				<div class="vertical-tabs">
					<a href="parent_acc.php">تفاصيل الحساب</a>
					<a href="p_std_my_result.php">نتائج الحفظ و المراجعة للابن</a>
					<a href="p_std_my_mton_result.php" class="active">نتائج حفظ و مراجعة المتون للابن</a>
			...
			<div class="content-main">
				<center style="margin-bottom:20px;"><div>
					<a href="p_std_mton_homework.php?std_id=<?php echo $_GET['std_id']; ?>" class="button" style="width:100px;height:50px;">
						<span>عرض الواجب</span>
					</a>
					<a href="p_std_my_mton_result.php" class="button" style="width:100px;height:50px;">
						<span>الرجوع</span>
					</a>
				</div></center>
				<div class="card-grid">
				...
				<center><table>
					...
					<?php
						$sql = "SELECT mton_std_att.state, students.std_name,mton_review.grade as rg1,mton_recite.grade as rg2,mton_review.date FROM students
								...
								LEFT JOIN mton_review ON mton_review.std_id = students.std_id
								LEFT JOIN mton_recite ON mton_recite.std_id = mton_review.std_id 
								INNER JOIN mton_std_att ON mton_std_att.std_id = mton_review.std_id and mton_std_att.date = mton_review.date
								WHERE parent.p_id ='".$_SESSION['email']."'  AND students.std_id = '".$_GET['std_id']."'
								ORDER BY date DESC"; 
							...
							while($row = mysqli_fetch_assoc($result)){
								if($row['date'] == date('Y-m-d')){
									echo '<tr >
											...
											<td style="background-color:#a3f3d2;">'.$row['rg1'].'</td>
											<td style="background-color:#a3f3d2;">'.$row['rg2'].'</td>
				...
				<?php }else{
						$sql = "SELECT *,mton_review_plan.amount as a1,mton_recite_plan.amount as a2 FROM students
								LEFT JOIN std_parent_rel ON std_parent_rel.std_id = students.std_id
								LEFT JOIN parent ON std_parent_rel.parent_id = parent.p_id and students.std_id = std_parent_rel.std_id
							...
							while($row = mysqli_fetch_assoc($result)){
								echo '<article class="card">
										...
										<div class="card-footer">
											<a href="p_std_my_mton_result.php?std_id='.$row['std_id'].'">مزيد من التفاصيل</a>
										</div>
									</article>
								';
							}
