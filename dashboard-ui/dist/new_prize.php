...
/* adding new prize */
	...
	if(isset($_POST['add_prize'])){
		...
		//if($row['ex'] != 1){
			$sql = "INSERT INTO `prize`(`prize_name`, `start_date`, `end_date`,`state`,`gold`,`silver`,`pronze`) VALUES ('".$name."','".$start_date."','".$end_date."','مستمرة','".$gold."','".$silver."','".$pronze."')";
			$result = mysqli_query($conn,$sql);
			if($result){
				echo "<script>alert('تمت إضافة الجائزة بنجاح');</script>";
				echo "<script>window.location.href='prize_man.php';</script>";
			}else{
				echo "<script>alert('خطأ في إضافة الجائزة')</script>";
			}
		/*}else{
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إضافة جائزة</title>
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="new_prize.php" method="post">
								<input type="text" placeholder="اسم الجائزة" name="name"/>
								<p style="font-size: 12px;width: 100%;">تاريخ بدء الجائزة : </p>
								<input type="date" placeholder="تاريخ بدء المتون" name="start_date"/>
								<p style="font-size: 12px;width: 100%;">تاريخ انتهاء الجائزة : </p>
								<input type="date" placeholder="تاريخ انتهاء المتون " name="end_date"/>
								<p style="font-size: 12px;width: 100%;">قيمة النجمة الذهبية : </p>
								<input type="text" placeholder="0" name="gold"/> 
								<p style="font-size: 12px;width: 100%;">قيمة النجمة الفضية : </p>
								<input type="text" placeholder="0" name="silver"/> 
								<p style="font-size: 12px;width: 100%;">قيمة النجمة البرونزية : </p>
								<input type="text" placeholder="0" name="pronze"/> 
								<button type="submit" name="add_prize">إضافة</button>
							  </form>
						</div>
					</article>					
				</div>
