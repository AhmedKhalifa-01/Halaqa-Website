	...
	if(isset($_POST['add_review_plan'])){
		...
		if($type == 'option1'){
			if($_POST['from'] != "" and $_POST['to'] != ""){
				...
				$to = $_POST['to'];
				$so = $_POST['so'];
				$sql = "DELETE FROM recite_plan WHERE std_id = '".$_GET['std_id']."'";
				mysqli_query($conn,$sql);
				$sql = "INSERT INTO `recite_plan`(`std_id`, `type`,`a_from`, `a_to`,`sora`) VALUES ('".$_GET['std_id']."','1','".$from."','".$to."','".$so."')";
		...
		if($type == 'option2'){
			if(isset($_POST['options'])){
				...
				$sql = "DELETE FROM std_recite_sora WHERE std_id = '".$_GET['std_id']."'";
				mysqli_query($conn,$sql);
				foreach ($options as $option) {
					$sql = "INSERT INTO `std_recite_sora`(`std_id`, `sorah`) VALUES ('".$_GET['std_id']."','".$option."')";
					mysqli_query($conn,$sql);
				}
				$sql = "DELETE FROM recite_plan WHERE std_id = '".$_GET['std_id']."'";
				mysqli_query($conn,$sql);
				$sql = "INSERT INTO `recite_plan`(`std_id`, `type`) VALUES ('".$_GET['std_id']."','2')";
				if(mysqli_query($conn,$sql)){
					echo "<script>window.location.href='std_ring_plan.php?ring_id=".$_GET['ring_id']."';</script>";
				}
			}else{
		...
		//if($type == 'option3'){
			if($_POST['amount'] != ""){
				...
				$sql = "DELETE FROM recite_plan WHERE std_id = '".$_GET['std_id']."'";
				mysqli_query($conn,$sql);
				$sql = "INSERT INTO `recite_plan`(`std_id`, `type`,`amount`) VALUES ('".$_GET['std_id']."','3','".$amount."')";
				if(mysqli_query($conn,$sql)){
					echo "<script>window.location.href='std_ring_plan.php?ring_id=".$_GET['ring_id']."';</script>";
				}
			}else{
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إضافة خطة المراجعة </title>
				...
				<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>إضافة خطة المراجعة</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="new_recite_plan.php?std_id=<?php echo $_GET['std_id']; ?>&ring_id=<?php echo $_GET['ring_id']; ?>" method="post">
