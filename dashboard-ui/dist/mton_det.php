						...
						<?php
							...
							if($result){
								while($row = mysqli_fetch_assoc($result)){
									echo '<tr>';
										echo '<td style="font-size:16px;white-space: normal;word-wrap: break-word;max-width: 150px;">'.$row['std_name'].'</td>';
										...
										if($row['status'] != 'إجازة'){
											if($row['state'] == "متوقف"){
												...
												if($std_date == date('Y-m-d')){
													echo '<td style="font-size:16px;background-color: darkgreen;color:#fff;"><div style="width:100%;height:100%;" onclick="location.href=\'all_std_mton_details.php?std_id='.$row['std_id'].'&ring_id='.$_GET['ring_id'].'\'" style="cursor:pointer;"><p style="font-size:16px;">تم تسجيل الدرجات اليوم</br>اضغط لاظهار التفاصيل</p></div></td>';
												}else{
													echo '<td style= "font-size:16px;background-color: #280950;color:#fff;"><a href="mton_details.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'" style="color:#fff;text-decoration:none;">اضغط لتسجيل الدرجات</a></td>';
												}
											}
										}else{
