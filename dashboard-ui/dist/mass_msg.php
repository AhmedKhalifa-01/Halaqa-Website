		...
		if(isset($_POST['msg'])){
			if($_GET['type'] == 0){
				...
				while($row = mysqli_fetch_assoc($result)){
					$sql = "INSERT INTO `messages`(`user_id`, `user_type`, `msg`, `date`) VALUES ('".$row['staff_id']."','".$_GET['type']."','".$msg."','".date('Y-m-d')."')";
			...
			if($_GET['type'] == 1){
				...
				$msg = $_POST['msg'];
				while($row = mysqli_fetch_assoc($result)){
					$sql = "INSERT INTO `messages`(`user_id`, `user_type`, `msg`, `date`) VALUES ('".$row['std_id']."','".$_GET['type']."','".$msg."','".date('Y-m-d')."')";
					mysqli_query($conn,$sql);
				}
			}
			if($_GET['type'] == 2){
				...
				$msg = $_POST['msg'];
				while($row = mysqli_fetch_assoc($result)){
					$sql = "INSERT INTO `messages`(`user_id`, `user_type`, `msg`, `date`) VALUES ('".$row['p_id']."','".$_GET['type']."','".$msg."','".date('Y-m-d')."')";
					mysqli_query($conn,$sql);
				}
			}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>رسالة جماعية</title>
	...
	<div class="header-logo">
			...
			<?php if($_SESSION['staff_job'] != "JOB_03"){?>
				<a href="new_acc.php">إضافة حساب</a>
					...
					?>
						<a href="std_management.php" style="background-color:darkred; color:white;">رسالة جماعية</a>
					<?php }else{?>
						<a href="std_management.php">إدارة الطلاب</a>
					<?php } ?>
					<a href="staff_management.php">إدار المعلمين</a>
					<?php } ?>
					<?php if($_SESSION['staff_job'] == "JOB_01"){?>
					...
					<?php
						if($_SESSION['staff_job'] == 'JOB_01'){
							echo '<a href="send_msg.php" class="active">إرسال رسالة</a>';
						}
					?>
			</br></br>
					...
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3 id="test">إرسال رسالة للمعلمين</h3>
							</div>
						</div>
						<div class="card-body">
						</div>
						<div class="card-footer">
							<a href="mass_msg.php?type=0">إرسال</a>
						</div>
					</article>
					<article class="card">
						<div class="card-header">
							<div>
								<span><img src="imgs/icons/adult.png" /></span>
								<h3 id="test">إرسال رسالة للإداريين</h3>
							</div>
						</div>
						<div class="card-body">
						</div>
						<div class="card-footer">
							<a href="mass_msg.php?type=3">إرسال</a>
						</div>
					</article>
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3 id="test">إرسال رسالة للطلاب</h3>
							</div>
						</div>
						<div class="card-body">
						</div>
						<div class="card-footer">
							<a href="mass_msg.php?type=1">إرسال</a>
						</div>
					</article>
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3 id="test">إرسال رسالة لأولياء الأمور</h3>
							</div>
						</div>
						<div class="card-body">
						</div>
						<div class="card-footer">
							<a href="mass_msg.php?type=2">إرسال</a>
						</div>
					</article>
					<?php }
						if(isset($_GET['type'])){
							echo '
									...
									<div class="card-body">
										<form class="login-form" action="mass_msg.php?type='.$_GET['type'].'" method="post">
