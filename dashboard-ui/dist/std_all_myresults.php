			...
			<table>
				...
				<?php
									//for ($i = 0; $i < count($dates); $i++) {
												//echo $dates[$i]. "</br> " ;
												$printedRev = array();
												$sql = "SELECT DISTINCT review.staff_id as teacher,recite.rev_id,review.id,students.std_id, COALESCE(review_plan.amount,'لا توجد خطة') as rpm, COALESCE(recite_plan.amount,'لا توجد خطة') as rpm2, students.std_name,review.date,std_att.state,review.grade as g1,recite.grade as g2,std_on_plan.onPlan as sta FROM ring
														...
														while($row = mysqli_fetch_assoc($result)){
															...
															if($row['rev_id'] == NULL){
																$sql = "SELECT DISTINCT review.id  as revId,students.std_id, COALESCE(review_plan.amount,'لا توجد خطة') as rpm, COALESCE(recite_plan.amount,'لا توجد خطة') as rpm2, students.std_name,review.date,std_att.state,review.grade as g1,recite.grade as g2,std_on_plan.onPlan as sta 
																		...
																		LEFT JOIN review_plan ON review_plan.std_id = students.std_id
																		LEFT JOIN recite_plan ON recite_plan.std_id = students.std_id
																		WHERE students.std_id = '".$_SESSION['email']."' AND ring.ring_id = '".$_GET['ring_id']."'
																				...
																				$row2 = mysqli_fetch_assoc($result2);
																					...
																					if (in_array($row2['revId'], $printedRev)) {
																						...
																						<tr>';
																							...
																							echo '<td>'.$row2['sta'].'</td>';
																							$sql = "SELECT COALESCE(std_homework_soras.sora,'لا توجد سورة') as sor,s_from,s_to,date FROM std_homework_soras WHERE std_id = '".$_SESSION['email']."' AND date = '".$row2['date']."' and type='review'";
																							...
																							$sql = "SELECT * FROM std_homework_soras WHERE std_id = '".$_SESSION['email']."' AND date = '".$row2['date']."' and type='recite'";
																							$rres = mysqli_query($conn,$sql);
																							if(mysqli_num_rows($rres) > 0){
																								echo '<td><ul>';
																								while($rrow2 = mysqli_fetch_assoc($rres)){
																		...
																		if(date('Y-m-d') == $row['date']){
																			...
																			<tr>
																				...
																				$sql = "SELECT COALESCE(std_homework_soras.sora,'لا توجد سورة') as sor,s_from,s_to,date FROM std_homework_soras 
																						WHERE rev_id = '".$row['id']."' AND std_id = '".$_SESSION['email']."' AND date = '".$row['date']."' and type='review'";
																				$rres = mysqli_query($conn,$sql);
		
																				$sqlPlan = "SELECT * FROM review_plan WHERE std_id = '".$_SESSION['email']."'";
																				$revPlan = "no";
																			...
																			$sqlPlan = "SELECT * FROM recite_plan WHERE std_id = '".$_SESSION['email']."'";
																					$recPlan = "no";
																					$resultPlan = mysqli_query($conn,$sqlPlan);
																					if(mysqli_num_rows($resultPlan) > 0){
																						$rowPlan = mysqli_fetch_assoc($resultPlan);
																			...
																			if($recPlan != "no"){
																				$sql = "SELECT * FROM std_homework_soras WHERE rev_id = '".$row['id']."' AND std_id = '".$_SESSION['email']."' AND date = '".$row['date']."' and type='recite'";
																				$rres = mysqli_query($conn,$sql);
																				if(mysqli_num_rows($rres) > 0){
																					echo '<td class="td"><ul>';
																					while($rrow = mysqli_fetch_assoc($rres)){
																		...
																		';
																			...
																			<tr>';
																				...
																				echo '<td>'.$row['sta'].'</td>';
																				$sql = "SELECT COALESCE(std_homework_soras.sora,'لا توجد سورة') as sor,s_from,s_to,date FROM std_homework_soras WHERE rev_id = '".$row['id']."' AND std_id = '".$_SESSION['email']."' and type='review'";
																				$rres = mysqli_query($conn,$sql);
																			...
																			if($recPlan != "no"){
																				$sql = "SELECT * FROM std_homework_soras WHERE rev_id = '".$row['id']."' AND std_id = '".$_SESSION['email']."' and type='recite'";
																				$rres = mysqli_query($conn,$sql);
																				if(mysqli_num_rows($rres) > 0){
																					echo '<td><ul>';
																					while($rrow = mysqli_fetch_assoc($rres)){
