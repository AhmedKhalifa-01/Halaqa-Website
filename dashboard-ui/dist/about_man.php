...
/* adding new post */
	...
	if(isset($_POST['add_pref'])){
		$preftxt = $_POST['preftxt'];
		mysqli_query($conn,"UPDATE about SET pref = ''");
		mysqli_query($conn,"UPDATE about SET pref = '".$preftxt."'");
	}
	if(isset($_POST['add_goal'])){
		$goaltxt = $_POST['goaltxt'];
		mysqli_query($conn,"UPDATE about SET goal = ''");
		mysqli_query($conn,"UPDATE about SET goal = '".$goaltxt."'");
	}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>نبذة عن المجمع</title>
					...
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>نبذة عن المجمع</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="about_man.php" method="post" enctype="multipart/form-data">
								<textarea name="preftxt" placeholder="<?php echo mysqli_fetch_assoc(mysqli_query($conn,"SELECT pref FROM about"))['pref']; ?>"></textarea>
								<button type="submit" name="add_pref">إضافة</button>
							  </form>
						</div>
					</article>	
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="about_man.php" method="post" enctype="multipart/form-data">
								<textarea name="goaltxt" placeholder="<?php echo mysqli_fetch_assoc(mysqli_query($conn,"SELECT goal FROM about"))['goal']; ?>"></textarea>
								<button type="submit" name="add_goal">إضافة</button>
							  </form>
						</div>
					</article>				
				</div>
