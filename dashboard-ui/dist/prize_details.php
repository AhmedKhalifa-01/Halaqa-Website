...
if((isset($_SESSION['email'])!= "") && (isset($_SESSION['staff_job']) == "JOB_01")) {
	...
	if(isset($_GET['closeCourse'])){
		$sql = "UPDATE course SET status = 'closed' WHERE c_id = ".$_GET['id']."";
		$result = mysqli_query($conn,$sql);
		if(!$result){
			echo "<script>alert('خطأ في إغلاق الدورة')</script>";
			echo "<script>window.location.href='course_man.php';</script>";
		}else{
			$sql = "SELECT course.type,students.std_id,students.std_name,course.c_name,SUM(mark) AS total FROM `course_results_details` INNER JOIN students ON course_results_details.std_id = students.std_id
								INNER JOIN course ON course_results_details.c_id = course.c_id
								WHERE course_results_details.c_id = '".$_GET['id']."'
								GROUP BY course_results_details.std_id
						;";		
			...
			while($row = mysqli_fetch_assoc($resultt)){
				...
				$sqlAtt = "SELECT COUNT(std_id) as total FROM `course_attendance` WHERE course_id = '".$_GET['id']."' AND std_id = '".$row['std_id']."';";
				...
				$sqlR = "INSERT INTO `course_results`(`course_id`, `std_id`, `total_mark`, `result`) VALUES ('".$_GET['id']."','".$row['std_id']."','".$row['total']."','".$result."')";
	...
	<div class="responsive-wrapper">
		...
		<div class="content">
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<?php
						$sql = "SELECT *,count(prize_participating_students.std_id) as std_count FROM prize 
						INNER JOIN prize_participating_students ON prize.prize_id = prize_participating_students.prize_id
						WHERE prize.prize_id = '".$_GET['prize_id']."'";
						$result = mysqli_query($conn,$sql);
						$row = mysqli_fetch_assoc($result);
								...
								echo '<article class="card">
										...
										<div class="card-body">
											...
											<p> عدد الطلبة المشاركين في الجائزة  : '.$row['std_count'].'</p>
											<p> جائزة النجم الذهبي  : '.$row['gold'].' ريال</p>
											<p> جائزة النجم الفضي  : '.$row['silver'].' ريال</p>
											<p> جائزة النجم البرونزي  : '.$row['pronze'].' ريال</p>
											<p> الحالة : '.$row['state'].'</p>
										</div>
										';
										if($row['state'] != 'closed'){
												$prev = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM prev WHERE staff_id = '".$_SESSION['email']."' AND prev_id = '12'")) > 0;
												if($_SESSION['staff_job'] == 'JOB_01' or $prev){
													echo '<div class="card-footer">
															<a href="prize_add_select_ring_std.php?prize_id='.$_GET['prize_id'].'"> اضافة طلبة للجائزة</a>
														</div>';
												}
												$prev = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM prev WHERE staff_id = '".$_SESSION['email']."' AND prev_id = '13'")) > 0;
												if($_SESSION['staff_job'] == 'JOB_01' or $prev){
													echo '<div class="card-footer">
														<a href="prize_del_std.php?prize_id='.$_GET['prize_id'].'"> حذف طالب من الجائزة</a>
													</div>';
												}
												$prev = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM prev WHERE staff_id = '".$_SESSION['email']."' AND prev_id = '14'")) > 0;
												if($_SESSION['staff_job'] == 'JOB_01' or $prev){
													echo '<div class="card-footer">
														<a href="add_prize_ring.php?prize_id='.$_GET['prize_id'].'"> اضافة حلقة للجائزة</a>
													</div>';
												}
										}else{
										...
										<div class="card-footer">
											<a href="show_prize_details.php?prize_id='.$_GET['prize_id'].'">عرض الطلبة المشاركين في الجائزة</a>
										</div>
									</article>
								';
