...
/* adding new mton */
	...
	if(isset($_POST['add_parent'])){
		if($_SESSION['staff_job'] == 'JOB_02'){
			...
			$email = $_POST['email'];
			$pass = $_POST['pass'];
			$sql = "INSERT INTO `pending_parent`(`p_name`, `p_id_number`, `p_phone`, `p_address`, `email`, `pass`) VALUES ('".$name."','".$phone."','".$phone."','".$address."','".$email."','".$pass."')";
			$result = mysqli_query($conn,$sql);
			if($result){
				echo "<script>alert('تمت إضافة ولي الأمر بنجاح، بانتظار الموافقة');</script>";
				echo "<script>window.location.href='parents_man.php';</script>";
			}else{
				echo "<script>alert('خطأ في إضافة ولي الأمر')</script>";
			}
		}else{
			$name = $_POST['name'];
			$num = $_POST['id_num'];
			...
			$email = $_POST['email'];
			$pass = $_POST['pass'];
			$sql = "INSERT INTO `parent`(`p_name`, `p_id_number`, `p_phone`, `p_address`, `email`, `pass`) VALUES ('".$name."','".$phone."','".$phone."','".$address."','".$email."','".$pass."')";
			$result = mysqli_query($conn,$sql);
			if($result){
				echo "<script>alert('تمت إضافة ولي الأمر بنجاح');</script>";
				echo "<script>window.location.href='parents_man.php';</script>";
			}else{
				echo "<script>alert('خطأ في إضافة ولي الأمر')</script>";
			}
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إضافة ولي أمر</title>
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="new_parent.php" method="post">
								<input type="text" placeholder="اسم ولي الأمر" name="name"/>
