...
/* changing password */
	if(isset($_POST['submit'])){
		...
		if($oldpass == mysqli_fetch_assoc($result)['pass']){
			if($pass == $rpass){
				$sql = "UPDATE `parent` SET `pass`='".$pass."' WHERE p_id = '".$_SESSION['email']."'";
				$result = mysqli_query($conn,$sql);
				if($result){
					echo "<script>alert('تم تغيير كلمة المرور بنجاح')</script>";
					echo "<script>window.location.href='parent_acc.php';</script>";
				}else{
		...
		<div class="content">
			<div class="content-panel">
				<div class="vertical-tabs">
					...
					<a href="p_std_my_rewards.php">سجل المكافآت الخاص بالابن</a>
					<a href="p_std_my_cm.php">الدورات التي تحصل الابن عليها</a>
					<a href="p_change_pwd.php" class="active">تغيير كلمة المرور</a>
					<a href="p_send.php">مراسلة المعلم</a>
					<?php
						$sql = "SELECT * FROM messages WHERE user_id = '".$_SESSION['email']."' AND user_type = 2 AND seen = 0";
						if((mysqli_num_rows(mysqli_query($conn,$sql)) > 0)){
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="p_change_pwd.php" method="post">
