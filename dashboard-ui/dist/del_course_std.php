...
/* checking if an admin is logged in */
	...
	if(isset($_POST['del'])){
		$sql = "DELETE FROM `course_details` WHERE std_id = '".$_GET['std_id']."' AND course_id = '".$_GET['c_id']."'";
		$result = mysqli_query($conn,$sql);
		if(!$result){
			echo "<script>alert('خطأ في حذف الطالب من الدورة')</script>";
			echo "<script>window.location.href='add_course_std.php?id=".$_GET['c_id']."';</script>";
		}else{
			echo "<script>alert('تم حذف الطالب من الدورة')</script>";
			//echo "<script>window.location.href='del_course_std.php?id=".$_GET['c_id']."';</script>";
		}
	}
					...
					<tbody>
						<?php
							$sql = "SELECT * FROM students
									INNER JOIN course_details ON students.std_id = course_details.std_id
							...
							if($result){
								while($row = mysqli_fetch_assoc($result)){
									echo '<tr>';
										...
										echo '<td>
												<form class="login-form" action="del_course_std.php?std_id='.$row['std_id'].'&c_id='.$_GET['c_id'].'" method="post">
													<input style="font-size:18px;font-weight:700;" type="submit" value="حذف" name="del" onclick="return confirmDel()"/>
												</form>
											</td>';
									echo '</tr>';
								}
...
<script>
		...
		function confirmDel() {
			if (confirm("هل أنت متأكد من حذف الطالب من الدورة ؟")) {
				return true;
			} else {
				return false;
			}
