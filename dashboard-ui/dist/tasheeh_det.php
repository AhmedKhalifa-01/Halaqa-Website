...
<main class="main">
	<div class="responsive-wrapper">
		...
		<div class="content">
			...
			<a href="tasheeh_man.php" class="button" style="margin-left:10px;width:100px;height:50px;">
					<span>رجوع</span>
				</a></br></br>
						...
						<?php
							...
							if($result){
								while($row = mysqli_fetch_assoc($result)){
									echo '<tr>';
										echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: #2d4650;color:#fff;"><div onclick="" style="cursor:pointer;"><p style="font-size:14px;">'.$row['std_name'].'</p></div></td>';
										...
										if($row['status'] != 'إجازة'){
											if($row['state'] == "متوقف"){
												...
												$sql = "SELECT COUNT(std_id) AS num FROM tasheeh_tasmee WHERE std_id = '".$row['std_id']."' AND date = '".date('Y-m-d')."'";
												$amountRev = mysqli_fetch_assoc(mysqli_query($conn,$sql))['num'];
												if($canDo){										
													switch ($amountRev) {
														case 0:
															echo '<td style= "background-color: #280950;color:#fff;white-space: normal;word-wrap: break-word;max-width: 150px;"><a href="tasheeh_tasmee.php?std_id='.$row['std_id'].'&t_id='.$_GET['t_id'].'" style="color:#fff;text-decoration:none;">التسميع</a></td>';
															break;
													case 1:
														echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: #34758f;color:#fff;"><div onclick="location.href=\'tasheeh_tasmee.php?std_id='.$row['std_id'].'&t_id='.$_GET['t_id'].'\'" style="cursor:pointer;"><p style="font-size:14px;">تم التسميع، </br> اضغط للتسميع للمرة الثانية</p></div></td>';
														break;
													case 2:
														echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: orange;color:#fff;"><div onclick="location.href=\'tasheeh_tasmee.php?std_id='.$row['std_id'].'&t_id='.$_GET['t_id'].'\'" style="cursor:pointer;"><p style="font-size:14px;">تم التسميع، </br> اضغط للتسميع للمرة الثالثة</p></div></td>';
														break;
													case 3:
														echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: purple;color:#fff;"><div onclick="location.href=\'tasheeh_tasmee.php?std_id='.$row['std_id'].'&t_id='.$_GET['t_id'].'\'" style="cursor:pointer;"><p style="font-size:14px;">تم التسميع، </br> اضغط للتسميع للمرة الرابعة</p></div></td>';
														break;
													case 4:
														echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: blue;color:#fff;"><div onclick="location.href=\'tasheeh_tasmee.php?std_id='.$row['std_id'].'&t_id='.$_GET['t_id'].'\'" style="cursor:pointer;"><p style="font-size:14px;">تم التسميع، </br> اضغط للتسميع للمرة الخامسة</p></div></td>';
														break;
													case 5:
														echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: darkblue;color:#fff;"><div onclick="location.href=\'tasheeh_tasmee.php?std_id='.$row['std_id'].'&t_id='.$_GET['t_id'].'\'" style="cursor:pointer;"><p style="font-size:14px;">تم التسميع، </br> اضغط للتسميع مرة أخيرة</p></div></td>';
														break;
													default:
										...
										echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: #742942;color:#fff;">
												<div onclick="location.href=\'old_tasheeh_details.php?std_id='.$row['std_id'].'&t_id='.$_GET['t_id'].'&days=7\'" style="cursor:pointer;">
													<p style="font-size:14px;">التصحيح القديم</p>
												</div>
											</td>';
											$plan = mysqli_fetch_assoc(mysqli_query($conn,"SELECT tp_plan FROM tasheeh_plan WHERE tp_std_id = '".$row['std_id']."'"))['tp_plan'];
										echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;;color:#000;">
...
<?php
    ...
    if(isset($_POST['add_sora'])){
        ...
        for ($i = 0; $i < count($courses); $i++) {
            ...
            echo "<script>window.location.href='ring_details.php?t_id=".$_GET['t_id']."';</script>";
        }
    }
