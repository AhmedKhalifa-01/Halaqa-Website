...
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title><?php
						if($_SESSION['staff_job'] == 'student'){
							$sql = "SELECT std_name as name FROM students WHERE std_id = '".$_GET['std_id']."'";
						}
						if($_SESSION['staff_job'] == 'parent'){
...
<main class="main">
	<div class="responsive-wrapper">
		...
		<div class="content">
			...
			<center style="margin-bottom:20px;"><div><a href="p_std_all_myresults.php?std_id=<?php echo $_GET['std_id']; ?>" class="button" style="width:100px;height:50px;">
					<span>الرجوع</span>
				</a></div></center>
				<div class="card-grid">
				<center><table>
					...
					<?php
						$sql = "SELECT mh_date FROM mton_homework WHERE std_id = '".$_GET['std_id']."' GROUP BY mh_date ORDER BY mh_date DESC LIMIT 7";
						$result=mysqli_query($conn,$sql);
						while($row = mysqli_fetch_assoc($result)){
							if(date('Y-m-d') == $row['mh_date']){
								echo "<tr>";
								$sql = "SELECT COALESCE(mton_homework.sora,'لا توجد سورة') as sor,mh_from,mh_to,mh_date 
										FROM mton_homework 
										WHERE std_id = '".$_GET['std_id']."' AND mh_type='review' AND mh_date = '".$row['mh_date']."'
										";
								...
								$sql = "SELECT COALESCE(mton_homework.sora,'لا توجد سورة') as sor,mh_from,mh_to,mh_date 
										FROM mton_homework 
										WHERE std_id = '".$_GET['std_id']."' AND mh_type='recite' AND mh_date = '".$row['mh_date']."'
										";
								$rres = mysqli_query($conn,$sql);
								if(mysqli_num_rows($rres) > 0){
									echo '<td class="td"><ul>';
								...
								echo '<td class="td">'.$row['mh_date'].'</td>';
								echo '<td class="td">'.getDayOfWeek($row['mh_date']).'</td>';
								echo "</tr>";
							}else{
								echo "<tr>";
								$sql = "SELECT COALESCE(mton_homework.sora,'لا توجد سورة') as sor,mh_from,mh_to,mh_date 
								...
								echo '<td>'.$row['mh_date'].'</td>';
								echo '<td>'.getDayOfWeek($row['mh_date']).'</td>';
								echo "</tr>";
							}
