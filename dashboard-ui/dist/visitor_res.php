...
/* move std to other ring */
	...
	if(isset($_POST['change'])){
		if(!isset($_POST['prev'])){
			mysqli_query($conn,"DELETE FROM prev WHERE staff_id = '".$_GET['staff_id']."'");
			echo "<script>alert('تم تعديل الصلاحيات');</script>";
			echo "<script>window.location.href='visitor_man.php';</script>";
			return;
		}
		$prev = $_POST['prev'];
		if (!empty($prev)) {
			if(mysqli_query($conn,"DELETE FROM prev WHERE staff_id = '".$_GET['staff_id']."'")){
				foreach ($prev as $pr) {
					if(mysqli_query($conn,"INSERT INTO `prev`(`staff_id`, `prev_id`) VALUES ('".$_GET['staff_id']."','".$pr."')")){
						echo "<script>alert('تم تعديل الصلاحيات');</script>";
						echo "<script>window.location.href='visitor_man.php';</script>";
					}
				}			
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إدارة الصلاحيات</title>
			...
			<?php if($_SESSION['staff_job'] != "JOB_03"){?>
					...
					<?php
						if($_SESSION['staff_job'] == 'JOB_01'){
							...
							echo '<a href=" visitor_man.php" class="active">إدارة الزوار</a>';
						}else{
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="visitor_res.php?staff_id=<?php echo $_GET['staff_id']; ?>" method="post">
								
								<p>اختر الصلاحيات : </p>
								<ul>
									<?php
									// Retrieve the list of students from the database
									$sql = "SELECT * FROM privileges WHERE id >= 100";
									$result = mysqli_query($conn, $sql);

									// Display the list of students as checkboxes
									if(mysqli_num_rows($result) > 0) {
										while($row = mysqli_fetch_assoc($result)) {
											$sql = "SELECT * FROM prev WHERE prev_id = '".$row['id']."' AND staff_id = '".$_GET['staff_id']."'";
		
											$res = mysqli_query($conn,$sql);
											if(mysqli_num_rows($res) > 0) {
												if(is_numeric($row['PREV'])){
													$ring_name = mysqli_fetch_assoc(mysqli_query($conn,"SELECT ring_name FROM ring WHERE ring_id = '".$row['PREV']."'"))['ring_name'];
													echo '<li><input style="width:auto;margin:10px;" type="checkbox" name="prev[]" value="'. $row["id"] .'" checked>عرض الحلقة : '.$ring_name.'</li>';
												}else{
													echo '<li><input style="width:auto;margin:10px;" type="checkbox" name="prev[]" value="'. $row["id"] .'" checked>'.$row['PREV'].'</li>';
												}
											}else{
												if(is_numeric($row['PREV'])){
													$ring_name = mysqli_fetch_assoc(mysqli_query($conn,"SELECT ring_name FROM ring WHERE ring_id = '".$row['PREV']."'"))['ring_name'];
													echo '<li><input style="width:auto;margin:10px;" type="checkbox" name="prev[]" value="'. $row["id"] .'">عرض الحلقة : '.$ring_name.'</li>';
												}else{
													echo '<li><input style="width:auto;margin:10px;" type="checkbox" name="prev[]" value="'. $row["id"] .'">'.$row['PREV'].'</li>';
												}
											}
										}
									}
								...
								<button type="submit" name="change">تعديل</button>
							  </form>
						</div>
					</article>					
				</div>
