...
<!-- partial:index.partial.html -->
		<center><h1>احصائيات الطالب</h1></center>
			<table>
												...
												if($result){
													if(mysqli_num_rows($result) > 0){
														while($row = mysqli_fetch_assoc($result)){
															...
															if($row['rev_id'] == NULL){
																$sql = "SELECT DISTINCT review.id  as revId,students.std_id, COALESCE(review_plan.amount,'لا توجد خطة') as rpm, COALESCE(recite_plan.amount,'لا توجد خطة') as rpm2, students.std_name,review.date,std_att.state,std_att.time,review.grade as g1,recite.grade as g2,std_on_plan.onPlan as sta 
																		FROM students
																		...
																		$row2 = mysqli_fetch_assoc($result2);
																			...
																			if (in_array($row2['revId'], $printedRev)) {
																				...
																				<tr>';
																					if($name == "null"){
																						$name = $row2['std_name'];
																						echo '<td rowspan="'.$days.'" style = "width:400px;">'.$row2['std_name'].'</td>';
																					}
																					echo '<td style = "width:600px;">'.$row2['date'].'</td>
																					<td style = "width:600px;">'.getDayOfWeek($row2['date']).'</br>'.$row2['time'].'</td>
																					<td>'.$row2['state'].'</td>';
																					if($row2['g1'] == ""){
																						echo '<td> - </td>';
																					}else{
				...
				<a href="old_ring_details.php?std_id=<?php echo $_GET['std_id']; ?>&ring_id=<?php echo $_GET['ring_id']; ?>&days=<?php $days = $days+7; echo $days; ?>"
