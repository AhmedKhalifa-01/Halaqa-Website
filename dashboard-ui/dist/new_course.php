...
/* adding new course */
	...
	if(isset($_POST['add_course'])){
		...
		$sql = "INSERT INTO `course`(`c_name`, `c_start_date`, `c_end_date`, `staff_id`, `status`,`type`) VALUES ('".$name."','".$start_date."','".$end_date."','00','مفتوحة','0')";
		...
		if($result){
			...
			if (!empty($staff)) {
				...
				foreach ($staff as $st) {
					$sql = "INSERT INTO `course_teachers`(`course_id`, `teacher_id`) VALUES ('".$last_inserted_id."','".$st."')";
					$result = mysqli_query($conn,$sql);
					if($result){
			...
			echo "<script>alert('خطأ في إضافة الدورة')</script>";
		}
	}
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="new_course.php" method="post" enctype="multipart/form-data">
								<p>اسم الدورة : </p>
								<input type="text" placeholder="اسم الدورة" name="name"/>
								<p>تاريخ بدء الدورة : </p>
								<input type="date" placeholder="تاريخ بدء الدورة" name="start_date"/>
								<p>تاريخ انتهاء الدورة : </p>
								<input type="date" placeholder="تاريخ انتهاء الدورة " name="end_date"/>
								<p>اختيار معلم الدورة : </p>
								<!-- <select name="staff" id="staff-select"> -->
									<?php
										$sql = "SELECT * FROM staff WHERE staff_job != 'JOB_01' AND staff_job != 'JOB_02' AND staff_job != 'JOB_04'";
										$result = mysqli_query($conn,$sql);
									...
									?>
								<!-- </select> -->
								<button type="submit" name="add_course">إضافة</button>
							  </form>
						</div>
					</article>					
				</div>
