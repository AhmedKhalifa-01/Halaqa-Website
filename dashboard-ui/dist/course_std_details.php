...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>تفاصيل الطلبة في الدورة</title>
...
<main class="main">
	<div class="responsive-wrapper">
		...
		<div class="content">
			...
			<center style="margin-bottom:20px;"><a href="course_details.php?id=<?php echo $_GET['c_id'];?>" class="button" style="width:100px;height:50px;">
					<span>رجوع</span>
				</a></center>
				<center><table style="width:320px;">
					<thead>
						<tr>
							<th>اسم الطالب</th>
							<th>إدخال الدرجة</th>
					...
					<tbody>
						<?php
							$sql = "SELECT * FROM students
									INNER JOIN course_details ON course_details.std_id = students.std_id
									WHERE course_details.course_id = '".$_GET['c_id']."'
							...
							if($result){
								while($row = mysqli_fetch_assoc($result)){
									echo '<tr>';
										echo '<td id = "std_name" style="font-size:16px;white-space: normal;word-wrap: break-word;max-width: 150px;">'.$row['std_name'].'</td>';
										if($row['status'] != 'إجازة'){
											echo '<td><a style="text-decoration:none" href="course_std_calc_result.php?std_id='.$row['std_id'].'&c_id='.$_GET['c_id'].'">إدخال الدرجة</a></td>';
											echo '<td><a style="text-decoration:none" href="std_change_att_details.php?std_id='.$row['std_id'].'&c_id='.$_GET['c_id'].'">تعديل الدرجة</a></td>';
										}else{
											echo '<td style="font-size:16px;background-color: orange;color:#fff;">الطالب في اجازة</td>';
											echo '<td style="font-size:16px;background-color: orange;color:#fff;">الطالب في اجازة</td>';
										}
										echo '<td><a style="text-decoration:none" href="course_std_res_details.php?std_id='.$row['std_id'].'&c_id='.$_GET['c_id'].'">عرض التفاصيل</a></td>';
									echo '</tr>';
								}
							}
						?>
