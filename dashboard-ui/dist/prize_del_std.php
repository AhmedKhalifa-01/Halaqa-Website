				...
				<?php 
							...
							if($_SESSION['staff_job'] == "JOB_03"){
								$sql = "SELECT ring.ring_id,ring.ring_name,t.staff_name AS teacher_name, s.staff_name AS supervisor_name, COUNT(students.std_id) AS student_count
										...
										INNER JOIN staff t ON t.staff_id = ring.staff_id
										INNER JOIN staff s ON s.staff_id = ring.ring_man
										WHERE ring.staff_id = '".$_SESSION['email']."'
										GROUP BY ring.ring_id
										ORDER BY ring.ring_name
										";
										if(isset($_GET['search'])){
							...
							while($row = mysqli_fetch_assoc($result)){
								$ringPart = false;
								$sql = "SELECT * FROM students WHERE ring_id = '".$row['ring_id']."'";
								$res = mysqli_query($conn,$sql);
								while($row2 = mysqli_fetch_assoc($res)){
									$sql = "SELECT * FROM prize_participating_students WHERE std_id = '".$row2['std_id']."'";
									...
									echo '<article class="card">
											...
											if($_SESSION['staff_job'] == 'JOB_01' or $prev){
												echo '<div class="card-footer">
														<a style= "color:red;" href="prize_del_select_std.php?&ring_id='.$row['ring_id'].'&prize_id='.$_GET['prize_id'].'"">عرض الطلبة المشاركين في الجائزة</a>
													</div>';
											}
