...
<html lang="ar" >
	<head>
	<meta charset="UTF-8">
	<link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>تعديل النجوم</title>
...
<center style="margin-bottom:20px;"><a href="show_prize_details.php?prize_id=<?php echo $_GET['prize_id'];?>" class="button" style="margin-top:100px;width:100px;height:50px;">
					<span>اغلاق</span>
				</a></center>
		<center><h1>تعديل النجوم</h1></center>
		
			<table>
				<thead>
					<tr>
						<th>اسم الطالب</th>
						<th>النجمة البرونزي</th>
						<th>النجمة الفضي</th>
						<th>النجمة الذهبي</th>
					</tr>
				</thead>
	...
	<?php
		...
		$sql = "SELECT 
					...
					COUNT(CASE WHEN pd.star = 'البرونزي' THEN 1 ELSE NULL END) AS BronzeStars,
					COUNT(CASE WHEN pd.star = 'الفضي' THEN 1 ELSE NULL END) AS SilverStars,
					COUNT(CASE WHEN pd.star = 'الذهبي' THEN 1 ELSE NULL END) AS GoldStars
				FROM students s
				...
				echo '
					<tr>
						...
						<td class="pronze" style="width:600px;">
							...
							<button class="add-star" data-student-id="'.$row['StudentID'].'" data-star-type="البرونزي" data-silver-rate="'.$bronze_to_silver_rate.'">+</button>
							<button class="remove-star" data-student-id="'.$row['StudentID'].'" data-star-type="البرونزي">-</button>
						</td>
...
<script>
	...
	var prize_id = urlParams.get('prize_id');
		...
		$('.add-star').click(function(){
			...
			if(star_count % conversion_rate == 0){
				...
				if(star_type == 'البرونزي'){
					...
					if(silver_count >= 2){
						...
						$.ajax({
							url: 'insert_star.php',
							method: 'POST',
							data: {student_id: student_id, star_type: 'الذهبي', date: date, prize_id: prize_id},
						...
						$.ajax({
							url: 'remove_star.php',
							method: 'POST',
							data: {student_id: student_id, star_type: 'الفضي', prize_id: prize_id, limit: 2},
							success: function(response){
								// handle success response
								console.log(response);
							},
						...
						$.ajax({
							url: 'remove_star.php',
							method: 'POST',
							data: {student_id: student_id, star_type: 'البرونزي', prize_id: prize_id, limit: 2},
							success: function(response){
								// handle success response
								console.log(response);
							},
						...
						$.ajax({
							url: 'insert_star.php',
							method: 'POST',
							data: {student_id: student_id, star_type: 'الفضي', date: date, prize_id: prize_id},
							success: function(response){
								// handle success response
								console.log(response);
							},
					...
					//if(gold_count >= 3){
						...
						//$(this).closest('tr').find('.silver .star-count').text(0);
						//addStar(student_id, 'الذهبي', date);
					//} else {
				...
				$.ajax({
					url: 'insert_star.php',
					method: 'POST',
					data: {student_id: student_id, star_type: star_type, date: date, prize_id: prize_id},
					success: function(response){
						// handle success response
						console.log(response);
					},
		...
		$('.remove-star').click(function(){
			...
			if(star_count == 0){
				...
				if(silver_count == 0){
					...
					if(gold_count != 0){
						$.ajax({
							url: 'remove_star.php',
							method: 'POST',
							data: { student_id: student_id, star_type: 'الذهبي', prize_id: prize_id, limit: 1 },
							success: function(response) {
					...
					$.ajax({
						url: 'remove_star.php',
						method: 'POST',
						data: { student_id: student_id, star_type: 'الفضي', prize_id: prize_id, limit: 1 },
						success: function(response) {
							// handle success response from first request
							console.log(response);
							// initiate second AJAX request
							$.ajax({
								url: 'insert_star.php',
								method: 'POST',
								data: { student_id: student_id, star_type: 'البرونزي', date: date, prize_id: prize_id },
								success: function(response) {
									// handle success response from second request
									console.log(response);
				...
				$.ajax({
					url: 'remove_star.php',
					method: 'POST',
					data: {student_id: student_id, star_type: star_type, prize_id: prize_id, limit: 1},
					success: function(response){
						// handle success response
						console.log(response);
					},
