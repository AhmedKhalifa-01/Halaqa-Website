...
/* checking if an admin is logged in */
	...
	if(isset($_POST['newNote'])){
		$note = $_POST['note'];
		if($note != "")
		mysqli_query($conn,"INSERT INTO `notes`(`note`) VALUES('".$note."')");
	}
	if(isset($_POST['std_info'])){
		...
		if(isset($_POST['att']) or $canSkipCheck){
			...
			$sql = "INSERT INTO `tasheeh_tasmee`(`std_id`, `grade`, `date`, `staff_id`) VALUES ('".$_GET['std_id']."','".$score1."','".date('Y-m-d')."','".$_SESSION['email']."')";
			if(mysqli_query($conn,$sql)){
				$last_inserted_id = mysqli_insert_id($conn);
				$sql = "INSERT INTO `tasheeh_att`(`state`, `tasmee_id`) VALUES ('".$att."','".$last_inserted_id."')";
				mysqli_query($conn,$sql);
				if(isset($_POST['notes']) && !empty($_POST['notes'])){
					$notes = $_POST['notes'];
					foreach($notes as $note){
						mysqli_query($conn,"INSERT INTO `tasheeh_notes`(`std_id`,`tasmee_id`,`note`) VALUES('".$_GET['std_id']."','".$last_inserted_id."','".$note."')");
					}
				}
				// Finishing old sora
				$sqlDateReview = "SELECT date FROM tasheeh_homework WHERE std_id = '".$_GET['std_id']."' ORDER BY date DESC LIMIT 1";
				$rDate = mysqli_fetch_assoc(mysqli_query($conn,$sqlDateReview))['date'];
				$sql = "UPDATE `tasheeh_homework` SET `state`= 0 WHERE std_id = '".$_GET['std_id']."' AND date = '".$rDate."' ";
				mysqli_query($conn,$sql);
				...
				if($_POST['div'] == "no"){
					...
					if(mysqli_num_rows($resultPlan) > 0){
						...
						$revPlan = $rowPlan['tp_plan'];
						if($revPlan == "0"){
							$revPlan = "no";
						}
					}
					if($revPlan != "no"){
						$sql = "SELECT tt_id FROM tasheeh_tasmee WHERE std_id = '".$_GET['std_id']."' ORDER BY tt_id DESC LIMIT 1,1";
						$revId = mysqli_fetch_assoc(mysqli_query($conn,$sql))['tt_id'];
						//echo "<script>alert('$revId')</script>";
						$sql = "SELECT * FROM tasheeh_homework WHERE std_id = '".$_GET['std_id']."' AND tasmee_id = '".$revId."'";
						$re = mysqli_query($conn,$sql);
						if(mysqli_num_rows($re) > 0){
							while($row2 = mysqli_fetch_assoc($re)){
								mysqli_query($conn,"
									INSERT INTO `tasheeh_homework`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`state`,`tasmee_id`) VALUES ('".$_GET['std_id']."','".$row2['sora']."','".$row2['s_from']."','".$row2['s_to']."','".date('Y-m-d')."','1','".$last_inserted_id."')
								");
							}
							$sql = "SELECT * FROM tasheeh_homework_face WHERE std_id = '".$_GET['std_id']."' AND tasmee_id = '".$revId."'";
							$resul = mysqli_query($conn,$sql);
							if(mysqli_num_rows($resul) > 0){
								$row2 = mysqli_fetch_assoc($resul);
								$des = $row2['face'];
									mysqli_query($conn,"
										INSERT INTO `tasheeh_homework_face`(`std_id`, `face`, `date`, `tasmee_id`) VALUES ('".$_GET['std_id']."','".$des."','".date('Y-m-d')."','".$last_inserted_id."')
									");
							}
						}else{
							$sql = "SELECT date FROM tasheeh_homework WHERE std_id = '".$_GET['std_id']."' AND tasmee_id IS NULL ORDER by date DESC LIMIT 1";
							$dda = mysqli_fetch_assoc(mysqli_query($conn,$sql))['date'];

							$sql = "SELECT * FROM tasheeh_homework_face WHERE std_id = '".$_GET['std_id']."' AND date = '".$dda."'";
							$re = mysqli_query($conn,$sql);
							if(mysqli_num_rows($re) > 0){
								while($row2 = mysqli_fetch_assoc($re)){
									mysqli_query($conn,"
								...
								if(mysqli_num_rows($re) > 0){
									...
									mysqli_query($conn,"
										INSERT INTO `tasheeh_homework_face`(`std_id`, `face`, `date`, `tasmee_id`) VALUES ('".$_GET['std_id']."','".$row2['face']."','".$row2['date']."','".$last_inserted_id."')"
									);
								}
							}
						}
					...
					if(isset($_POST['courses'])){
						...
						for ($i = 0; $i < count($courses); $i++) {
							...
							if($course != "none"){
								$sql = "INSERT INTO `tasheeh_homework`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`state`,`tasmee_id`) VALUES ('".$_GET['std_id']."','".$course."','".$page1_range."','".$page2_range."','".date('Y-m-d')."','1','".$last_inserted_id."')";
						...
						if(isset($_POST['sora'])){
							...
							for ($i = 0; $i < count($courses); $i++) {
								...
								if($course != "none"){
									...
									$sql = "INSERT INTO `tasheeh_homework`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`state`,`tasmee_id`) VALUES ('".$_GET['std_id']."','".$course."','-','-','".date('Y-m-d')."','1','".$last_inserted_id."')";
									$result = mysqli_query($conn,$sql);
									if(!$result){
										echo mysqli_error($conn);
									}
						...
						if(isset($_POST['numberSelector'])){
							...
							$sql = "INSERT INTO `tasheeh_homework_face`(`std_id`, `face`, `date`, `tasmee_id`) VALUES ('".$_GET['std_id']."','".$val."','".date('Y-m-d')."','".$last_inserted_id."')";
							mysqli_query($conn,$sql);
						}
				}else{
					if(isset($_POST["sora"])){
		...
		echo "<script>window.location.href='tasheeh_det.php?t_id=".$_GET['t_id']."';</script>";
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>تسميع التصحيح</title>
	...
	<div class="responsive-wrapper">
		<div class="content-header">
			<div class="content-header-intro">
				<h2>
					<?php
						$rname = mysqli_fetch_assoc(mysqli_query($conn,"SELECT t_name FROM tasheeh WHERE id = '".$_GET['t_id']."'"))['t_name'];
						echo $rname;
					?>
				</h2>
		...
		<div class="content">
			...
			<center style="margin-bottom:20px;"><a href="javascript:history.go(-1)" id="backbtn" class="button" style="width:100px;height:50px;">
				...
				<?php if(isset($_GET['std_id'])){?>
						<form class="login-form" id="myForm" action="tasheeh_tasmee.php?t_id=<?php echo $_GET['t_id'];?>&std_id=<?php echo $_GET['std_id'];?>" method="post" onsubmit="return validateForm()">
								...
								<input style="width:auto;margin:10px;display:inline-block;" type="radio" name="att" value="مستاذن" id="disp2" onclick="disapleHomework()"><label for="disp2">مستاذن</label></br></br>
									...
									<?php
										...
										if(mysqli_num_rows($resultPlan) > 0){
											...
											if($revPlan == "0"){
												$revPlan = "no";
												echo '<h1 style="font-size:19px;">خطة التصحيح : <color style="color:#b40836">متوقف</color></h1></br>';

											}else{
												echo '<h1 style="font-size:19px;">خطة التصحيح : <color style="color:#b40836">'.$rowPlan['tp_plan'].'</color></h1></br>';
											}
											//  خط الحفظ
										}else{
											
											echo '<h1 style="font-size:19px;color:#b40836;">الطالب ليس لديه خطة تصحيح</h1></br>';
										}
										...
										echo '<h1 style="font-size:19px;">واجب التصحيح : </h1>';
										if($revPlan != "no"){
											$sqlDateReview = "SELECT date FROM tasheeh_homework WHERE std_id = '".$_GET['std_id']."' AND state = '1' ORDER BY date DESC LIMIT 1";
												$rowRDate = mysqli_fetch_assoc(mysqli_query($conn,$sqlDateReview));
												$sql = "SELECT * FROM tasheeh_homework WHERE std_id = '".$_GET['std_id']."'AND date = '".$rowRDate['date']."' AND state = '1'";
												...
												if(mysqli_num_rows($rres) > 0){
													...
													$sqlDateReview = "SELECT date FROM tasheeh_homework WHERE std_id = '".$_GET['std_id']."' ORDER BY date DESC LIMIT 1";
													$rowRDate = mysqli_fetch_assoc(mysqli_query($conn,$sqlDateReview));
													$sql = "SELECT * FROM tasheeh_homework WHERE std_id = '".$_GET['std_id']."' AND date = '".$rowRDate['date']."'";
													$rres = mysqli_query($conn,$sql);
													if(mysqli_num_rows($rres) > 0){
														echo '<center><table style="margin-bottom:10px;width:250px;border:1px solid black;"><tr>';
														$count = 0;
														...
														echo '<h1 style="font-size:18px;color:#1c9300;">الطالب ليس لديه واجب تصحيح</h1></br>';
													}
												}
											}else{
												echo '<h1 style="font-size:18px;color:#1c9300;">الطالب ليس لديه واجب تصحيح</h1></br>';
										...
										?>
										<div id="select_note" style="">
											<p style="font-size:14px;">اختر الملاحظات او قم باضافة ملاحظة جديدة : </p>
											<select name="notes[]" id="notes" style="max-width:235px;height:50px;float:right;" multiple>
												<?php
													$noteResult = mysqli_query($conn,"SELECT * FROM notes ORDER BY note_id");
													while($noteRow = mysqli_fetch_assoc($noteResult)){
														echo '<option value="'.$noteRow['note_id'].'" style="text-wrap: pretty;">'.$noteRow['note'].'</option>';
													}
												?>
											</select>
											<button type="button" style="float:left;margin-top:5px;text-align:center;width:40px;padding:0px; height:40px;" onclick="hideSelectNote()">+</button>
										</div>
										<div id="new_note" style="margin-top:10px;display:none;">
											<p style="font-size:14px;">اضافة ملاحظة جديدة : </p>
											<input type="text" name="note" style="margin-top:0px;width:240px;height:30px;"/>
											<button onclick="skipCheck();" type="submit" name="newNote" style="float:left;margin-top:0px;text-align:center;width:30px;padding:0px; height:30px;">+</button>
										</div>
										<script>
											var skipC = false;
											function hideSelectNote(){
												var select_note = document.getElementById("select_note");
												select_note.style.display = "none";
									...
									<center><table id = "score_table" style="margin-top:60px;">
									...
									<tr>
											...
											<?php
												if($revPlan == 'no'){
											?>
												<td span=5>لا توجد خطة للتصحيح</td>
											<?php
												}else{
											?>
												<td>
						...
						<article class="card" id="homework" style="margin-top:10px">
							...
							<div class="card-body">
								...
								?>
								<h2 style="font-weight:bold;">واجب التصحيح </h2>
								<center>
		...
		function validateForm() {
			...
			if(!disp.checked && !disp2.checked){
				if(revPlan.innerHTML != "no"){
					if (!option1.checked && !option2.checked 
						&& !option3.checked && !option4.checked && !skipC){
							event.preventDefault();
						alert("الرجاء اختيار درجة التصحيح");
					}
				}
			}
			...
			if(rad1.checked){
				...
				if (pages1.value === "" || pages2.value === "" || course.value === "") {
					...
					if(!skipC){
						event.preventDefault();
						alert("الرجاء اختيار السورة والآيات للتصحيح");
					}
				}
				if (selectedNumber.value === "" && selectedQuarterHalf.value === "0") {
					...
					if(!skipC){
						event.preventDefault();
						alert("الرجاء اختيار عدد الأوجه للتصحيح");
					}
				}
			}
			if(rad2.checked){
