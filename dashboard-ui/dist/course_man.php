...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>إدارة الدورات</title>
	...
	<div class="responsive-wrapper">
		...
		<div class="content-header">
			<div class="content-header-intro">
				<h2>إدارة الدورات</h2>
				<?php if($_SESSION['staff_job'] == 'JOB_01'){ ?>
				<center><h3 style="font-weight:bold;font-size:24px;">عدد الدورات المفتوحة في المجمع : <color style="color:#ffffff;"><?php 
														$res = mysqli_query($conn,"SELECT COUNT(c_id) AS count FROM course WHERE status = 'مفتوحة'"); 
			...
			<div class="content-header-actions">
				...
				<?php
					if($_SESSION['staff_job'] == 'JOB_01'){
						echo '<a href="new_course.php" class="button">
								<i class="ph-plus-bold"></i>
								<span>إضافةدورة جديدة</span>
							</a>';
							echo '<a href="cm_stat.php" class="button">
								<i class="ph-plus-bold"></i>
								<span>احصائيات الدورات</span>
							</a>';
					}
				?>
			</div>
		...
		<div class="content">
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<?php
					$sql = "SELECT * FROM course WHERE type = 0 AND status = 'مفتوحة'";
					if($_SESSION['staff_job'] == "JOB_03"){
						$sql = "SELECT * FROM course WHERE staff_id = '".$_SESSION['email']."' WHERE type = 0 AND status = 'مفتوحة'";
					}
						...
						while($row = mysqli_fetch_assoc($result)){
								echo '<article class="card">
										...
										<div class="card-body">
											...
											<p> الحالة : '.$row['status'].'</p>
										</div>
										<div class="card-footer">
											<a href="course_details.php?id='.$row['c_id'].'">مزيد من التفاصيل</a>
										</div>
									</article>
								';
