...
if((isset($_SESSION['email'])!= "") && (isset($_SESSION['staff_job']) == "JOB_01")) {
	...
	if(isset($_POST['att_good'])){
		$att_date = $_POST['att_date'];
		$sql = "INSERT INTO `course_attendance`(`course_id`, `std_id`, `date`, `state`) VALUES ('".$_GET['id']."','".$_GET['std_id']."','".$att_date."','حضور')";
		$result = mysqli_query($conn,$sql);
		if($result){

		}else{
			echo "<script>alert('حدث خطأ في إضافة الحضور');</script>";
		}
	}
	if(isset($_POST['att_bad'])){
		$att_date = $_POST['att_date'];
		$sql = "INSERT INTO `course_attendance`(`course_id`, `std_id`, `date`, `state`) VALUES ('".$_GET['id']."','".$_GET['std_id']."','".$att_date."','غياب')";
		$result = mysqli_query($conn,$sql);
		if($result){
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>حضور الطلبة</title>
...
<?php 
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<?php
						$sql = "SELECT * FROM course_details WHERE course_id = ".$_GET['id']."";
						$result = mysqli_query($conn,$sql);
						while($row = mysqli_fetch_assoc($result)){
							$sqlStd = "SELECT * FROM students WHERE std_id = ".$row['std_id']."";
							...
							echo '<article class="card">
									...
									<div class="card-body">
										<p> الإيميل : '.$rowStd['email'].'</p>
										<p> رقم الهاتف : '.$rowStd['std_phone'].'</p>
										<p> رقم هاتف ولي الأمر : '.$rowStd['std_parent_phone'].'</p>
									</div>
									<div class="card-footer">';

									$today = date('Y-m-d');
									$sql2 = "SELECT * FROM course_attendance WHERE course_id = '".$_GET['id']."' AND std_id = '".$rowStd['std_id']."' AND date = '".$today."'";
									$result2 = mysqli_query($conn,$sql2);
									$row2 = mysqli_fetch_assoc($result2);
									if($row2){
										echo '<p>حالة الحضور اليوم: '.$row2['state'].'</p></div>';
										echo '<div class="card-footer">
										<a href="std_att_details.php?id='.$_GET['id'].'&std_id='.$rowStd['std_id'].'" target="_blank">عرض تفاصيل حضور الطالب</a></div>
										<div class="card-footer">
											<a href="course_details.php?id='.$_GET['id'].'">الرجوع</a>
										</div>
										</article>';
									}else{
										$sqlCourse = "SELECT * FROM course WHERE c_id = '".$_GET['id']."'";
										$resultCourse = mysqli_query($conn,$sqlCourse);
										$rowCourse = mysqli_fetch_assoc($resultCourse);
										if($rowCourse['status'] == 'مفتوحة'){
											echo '<div class="card-footer"><form class="login-form" action="add_std_attendance_course.php?id='.$_GET['id'].'&std_id='.$rowStd['std_id'].'" method="post" enctype="multipart/form-data">
														<input type="date" value = "'.date('Y-m-d').'"placeholder="تاريخ الحضور " name="att_date"/>
														<button type="submit" name="att_good">حضور</button>
														<button type="submit" name="att_bad">غياب</button>
													</form>
													</div>
												</article>
											';
										}else{
											echo '<p">الدورة مغلقة</p>';
											echo '<div class="card-footer">
													<a href="std_att_details.php?id='.$_GET['id'].'&std_id='.$rowStd['std_id'].'">عرض تفاصيل حضور الطالب</a>
												</div>';
												echo '<div class="card-footer">
													<a href="course_details.php?id='.$_GET['id'].'">الرجوع</a>
												</div>
