...
/* adding new ring */
	...
	if(isset($_POST['adj'])){
		...
		$sql = "UPDATE ring SET `ring_name` = '".$ring_name."' WHERE ring_id = '".$_GET['ring_id']."'";
		mysqli_query($conn,$sql);
		
		echo "<script>alert('تمت تعديل الحلقة بنجاح');</script>";
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>التعديل على الحلقة</title>
					...
					<article class="card">
						<div class="card-header">
							<div>
								...
								<h3>التعديل على الحلقة</h3>
							</div>
						</div>
						<div class="card-body">
							<form class="login-form" action="adj_ring.php?ring_id=<?php echo $_GET['ring_id']; ?>" method="post">
								<p>اسم الحلقة</p>
								<input type="text" placeholder="اسم الحلقة" name="ring_name">
								<button type="submit" name="adj">تعديل</button>
							  </form>
						</div>
					</article>					
				</div>
