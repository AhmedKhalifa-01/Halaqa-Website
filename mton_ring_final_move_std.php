	...
	if (!empty($students)) {
		foreach ($students as $std_id) {
			...
			if($row['mton_ring_id'] == $_GET['mton_ring_id']){
					...
					mysqli_query($conn,"UPDATE students SET mton_ring_id = '".$ring."', mton_ring_temp_id = '".$ring."' WHERE std_id = '".$std_id."'
										AND mton_ring_id_02 != '".$ring."' AND mton_ring_id_03 != '".$ring."'");

			}else if($row['mton_ring_id_02'] == $_GET['mton_ring_id']){

					mysqli_query($conn,"UPDATE students SET mton_ring_id_02 = '".$ring."', mton_ring_temp_id_02 = '".$ring."' WHERE std_id = '".$std_id."'
										AND mton_ring_id != '".$ring."' AND mton_ring_id_03 != '".$ring."'");
			}else if($row['mton_ring_id_03'] == $_GET['mton_ring_id']){

					mysqli_query($conn,"UPDATE students SET mton_ring_id_03 = '".$ring."', mton_ring_temp_id_03 = '".$ring."' WHERE std_id = '".$std_id."'
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="mton_ring_final_move_std.php?mton_ring_id=<?php echo $_GET['mton_ring_id']; ?>" method="post">
								...
								<p>اختر الطلبة لنقلهم من الحلقة :</br><b style="font-size:14px;"> (الطلبة المحولين مؤقتا لا يظهرن في هذه القائمة)</b></p>
								<ul>
									<?php
									...
									$sql = "SELECT students.std_id,students.mton_ring_id,students.mton_ring_temp_id, students.mton_ring_id_02,students.mton_ring_temp_id_02, students.mton_ring_id_03,students.mton_ring_temp_id_03, mton_ring.mton_ring_id,students.std_name FROM mton_ring 
											...
											WHERE students.mton_ring_id = '".$_GET['mton_ring_id']."'
											OR students.mton_ring_id_02 = '".$_GET['mton_ring_id']."'
											OR students.mton_ring_id_03 = '".$_GET['mton_ring_id']."'
											ORDER BY std_name";
									$result = mysqli_query($conn, $sql);
								...
								<button type="submit" name="move_std">نقل</button>
							  </form>
						</div>
					</article>					
				</div>
