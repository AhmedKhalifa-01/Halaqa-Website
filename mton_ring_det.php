...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>تسميع المتون</title>
...
<main class="main">
	<div class="responsive-wrapper">
		...
		<div class="content">
			...
			<a href="mton_man.php" class="button" style="margin-left:10px;width:100px;height:50px;">
					<span>رجوع</span>
				</a></br></br>
				...
				<center><table style="">
					...
					<tbody>
						<?php
							$sql = "SELECT students.state,students.status,students.std_id, students.std_name FROM students
									INNER JOIN mton_ring ON mton_ring.mton_ring_id = students.mton_ring_temp_id OR mton_ring.mton_ring_id = students.mton_ring_temp_id_02 OR mton_ring.mton_ring_id = students.mton_ring_temp_id_03
									WHERE mton_ring.mton_ring_id = ".$_GET['mton_ring_id']."
									";
							$result = mysqli_query($conn,$sql);
							if($result){
								while($row = mysqli_fetch_assoc($result)){
									echo '<tr>';
										echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: #2d4650;color:#fff;"><div style="cursor:pointer;"><p style="font-size:14px;">'.$row['std_name'].'</p></div></td>';
										...
										if($row['status'] != 'إجازة'){
											if($row['state'] == "متوقف"){
												...
												$sql = "SELECT COUNT(std_id) AS num FROM mton_review WHERE std_id = '".$row['std_id']."' AND date = '".date('Y-m-d')."'";
												$amountRev = mysqli_fetch_assoc(mysqli_query($conn,$sql))['num'];
												if($canDo){										
													switch ($amountRev) {
														case 0:
															echo '<td style= "background-color: #280950;color:#fff;white-space: normal;word-wrap: break-word;max-width: 150px;"><a href="mton_details.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'" style="color:#fff;text-decoration:none;">التسميع</a></td>';
															break;
													case 1:
														echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: #34758f;color:#fff;"><div onclick="location.href=\'mton_details.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'\'" style="cursor:pointer;"><p style="font-size:14px;">تم التسميع، </br> اضغط للتسميع للمرة الثانية</p></div></td>';
														break;
													case 2:
														echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: orange;color:#fff;"><div onclick="location.href=\'mton_details.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'\'" style="cursor:pointer;"><p style="font-size:14px;">تم التسميع، </br> اضغط للتسميع للمرة الثالثة</p></div></td>';
														break;
													case 3:
														echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: purple;color:#fff;"><div onclick="location.href=\'mton_details.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'\'" style="cursor:pointer;"><p style="font-size:14px;">تم التسميع، </br> اضغط للتسميع للمرة الرابعة</p></div></td>';
														break;
													case 4:
														echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: blue;color:#fff;"><div onclick="location.href=\'mton_details.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'\'" style="cursor:pointer;"><p style="font-size:14px;">تم التسميع، </br> اضغط للتسميع للمرة الخامسة</p></div></td>';
														break;
													case 5:
														echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: darkblue;color:#fff;"><div onclick="location.href=\'mton_details.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'\'" style="cursor:pointer;"><p style="font-size:14px;">تم التسميع، </br> اضغط للتسميع مرة أخيرة</p></div></td>';
														break;
													default:
										...
										echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;background-color: #742942;color:#fff;">
												<div onclick="location.href=\'old_mton_ring_details.php?std_id='.$row['std_id'].'&mton_ring_id='.$_GET['mton_ring_id'].'\'" style="cursor:pointer;">
...
<?php
    ...
    if(isset($_POST['add_sora'])){
        ...
        for ($i = 0; $i < count($courses); $i++) {
			...
			$sql = "INSERT INTO `std_homework_soras`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`type`) VALUES ('".$_GET['stud_id']."','".$course."','".$page1_range."','".$page2_range."','".date('Y-m-d')."','mton_review')";
            $result = mysqli_query($conn,$sql);
			if(!$result){
				echo mysqli_error($conn);
			}
            echo "<script>window.location.href='mton_details.php?mton_ring_id=".$_GET['mton_ring_id']."';</script>";
        }
    }
	// mton_recite std_homework_soras
	if(isset($_POST['add_sora_rec'])){
        ...
        for ($i = 0; $i < count($courses); $i++) {
            ...
            $page1_range = $conn->real_escape_string($pages1[$i]);
			$page2_range = $conn->real_escape_string($pages2[$i]);
			$sql = "INSERT INTO `std_homework_soras`(`std_id`, `sora`, `s_from`, `s_to`, `date`,`type`) VALUES ('".$_GET['stud_id']."','".$course."','".$page1_range."','".$page2_range."','".date('Y-m-d')."','mton_recite')";
            $result = mysqli_query($conn,$sql);
			if(!$result){
				echo mysqli_error($conn);
			}
