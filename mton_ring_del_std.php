		...
		if(isset($_POST['students'])){
			...
			if (!empty($students)) {
				foreach ($students as $std_id) {
					mysqli_query($conn,"UPDATE students SET mton_ring_temp_id = NULL, mton_ring_id = NULL,mton_ring_temp_id_02 = NULL, mton_ring_id_02 = NULL,mton_ring_temp_id_03 = NULL, mton_ring_id_03 = NULL WHERE std_id = '".$std_id."'");
					echo "<script>alert('تم حذف الطالب بنجاح');</script>";
					echo "<script>window.location.href='mton_man.php';</script>";
				}
			}
		}
		/*if(isset($_POST['mton_ring'])){
			$ring = $_POST['ring'];
		}*/
...
<head>
<meta charset="UTF-8">
  <link rel="icon" sizes="120x120" href="../../images/lggo.png">  <title>حذف طالب من المتن</title>
			...
			<div class="content-main">
				...
				<div class="card-grid">
					<article class="card">
						...
						<div class="card-body">
							<form class="login-form" action="mton_ring_del_std.php" method="post">
								
								<p>اختر الطلبة لحذفهم من الحلقة : </p>
								<ul>
									<?php
									// Retrieve the list of students from the database
									$sql = "SELECT mton_ring.mton_ring_id,students.mton_ring_temp_id,students.state,students.status,students.std_id, students.std_name FROM students
									...
									if(mysqli_num_rows($result) > 0) {
										while($row = mysqli_fetch_assoc($result)) {
												if($row['mton_ring_id'] != $row['mton_ring_temp_id']){
													echo '<li><input style="width:auto;margin:10px;" type="checkbox" name="students[]" value="'. $row["std_id"] .'">'. $row["std_name"] .'   (طالب محول)</li>';
												}else{
