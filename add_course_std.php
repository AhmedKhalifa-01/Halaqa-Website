...
/* checking if an admin is logged in */
	...
	$sql = "SELECT * FROM course WHERE c_id = '".$_GET['c_id']."'";
	if(mysqli_num_rows(mysqli_query($conn,$sql)) == 0){
		//echo "<script>window.location.href='course_man.php';</script>";
	}
	if(isset($_POST['add'])){
		$sql = "INSERT INTO `course_details`(`course_id`, `std_id`) VALUES ('".$_GET['c_id']."','".$_GET['std_id']."')";
		$result = mysqli_query($conn,$sql);
		if(!$result){
			echo "<script>alert('خطأ في إضافة الطالب للدورة')</script>";
			echo "<script>window.location.href='add_course_std.php?c_id=".$_GET['c_id']."';</script>";
		}else{
			//echo "<script>alert('تمت إضافة الطالب في الدورة')</script>";
			echo "<script>window.location.href='add_course_std.php?c_id=".$_GET['c_id']."';</script>";
		}
	}
...
<style>
	...
	tbody td {
		 /* Set padding-top to the height of the fixed header */
	}
</style>
</head>
<body>
						...
						<?php
							...
							if($result){
								while($row = mysqli_fetch_assoc($result)){
									echo '<tr>';
										echo '<td style="white-space: normal;word-wrap: break-word;max-width: 150px;">'.$row['std_name'].'</td>';
										$sql = "SELECT * FROM course_details WHERE std_id = '".$row['std_id']."' AND course_id = '".$_GET['c_id']."'";
										if(mysqli_num_rows(mysqli_query($conn,$sql)) > 0 and $_SESSION['staff_job'] == 'JOB_01' ){
											echo '<td style = "background-color:darkred;color:#fff;">
													الطالب مسجل مسبقا
												</td>';
										}else{
											echo '<td>
													<form class="login-form" action="add_course_std.php?std_id='.$row['std_id'].'&c_id='.$_GET['c_id'].'" method="post">
														<input style="font-weight:700;" type="submit" value="إضافة" name="add"/>
													</form>
												</td>';
										}
									echo '</tr>';
